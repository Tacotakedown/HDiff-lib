...
#define _check_pthread(result, func_name)                                      \
  {                                                                            \
    if (result != 0)                                                           \
      throw std::runtime_error(func_name "() return " + _i2a(result) +         \
                               " error!");                                     \
  }
...
#if (_IS_USED_CPP11THREAD)
HLocker locker_new(void) { return new std::mutex(); }
void locker_delete(HLocker locker) {
  if (locker != 0)
...
#define _check(value, func_name)                                               \
  {                                                                            \
    if (!(value))                                                              \
      throw std::runtime_error(func_name "() run error!");                     \
  }
...
void condvar_wait(HCondvar cond, TLockerBox *lockerBox) {
  CONDITION_VARIABLE *self = (CONDITION_VARIABLE *)cond;
  BOOL rt = SleepConditionVariableCS(
      self, (CRITICAL_SECTION *)(lockerBox->locker), INFINITE);
...
void this_thread_yield() { Sleep(0); }
