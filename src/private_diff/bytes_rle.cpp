...
static void rle_pushSame(std::vector<TByte> &out_ctrl,
                         std::vector<TByte> &out_code, TByte cur, TUInt count) {
  assert(count > 0);
...
static inline void rle_pushNotSameCtrl(std::vector<TByte> &out_ctrl,
                                       TUInt count) {
  packUIntWithTag(out_ctrl, (TUInt)(count - 1), kByteRleType_unrle,
                  kByteRleType_bit);
}
...
struct TStreamRLE {
  ...
  inline explicit TStreamRLE(std::vector<unsigned char> &_ctrlBuf,
                             std::vector<unsigned char> &_codeBuf,
                             int rle_parameter)
      : ctrlBuf(_ctrlBuf), codeBuf(_codeBuf), sameCur(0), sameCount(0),
        notSameCount(0), kRleMinSameSize(rle_parameter + 1) {
    assert((rle_parameter >= kRle_bestSize) &&
           (rle_parameter <= kRle_bestUnRleSpeed));
  }
  ...
  inline void endSame() {
    size_t sign = ((size_t)(253 - (size_t)(TByte)(sameCur - 1))) >>
                  (sizeof(size_t) * 8 - 1);
...
void bytesRLE_save(std::vector<TByte> &out_ctrlBuf,
                   std::vector<TByte> &out_codeBuf, const TByte *src,
                   const TByte *src_end, int rle_parameter) {
...
void bytesRLE_save(std::vector<TByte> &out_code, const TByte *src,
                   const TByte *src_end, int rle_parameter) {
...
void bytesRLE_save(std::vector<TByte> &out_code, const hpatch_TStreamInput *src,
                   int rle_parameter) {
  std::vector<TByte> ctrlBuf;
  std::vector<TByte> codeBuf;
...
inline void _out_uncompressData(TSingleStreamRLE0 &self) {
  ...
  while (self.uncompressData.size() - saved > kMaxBytesRleLen) {
    ...
    pushBack(self.fixed_code, self.uncompressData.data() + saved,
             self.uncompressData.data() + saved + kMaxBytesRleLen);
    packUInt(self.fixed_code, 0);
    saved += kMaxBytesRleLen;
  }
  packUInt(self.fixed_code, self.uncompressData.size() - saved);
  pushBack(self.fixed_code, self.uncompressData.data() + saved,
           self.uncompressData.data() + self.uncompressData.size());
  self.uncompressData.clear();
}
inline hpatch_StreamPos_t
_out_uncompressData_codeSize(hpatch_StreamPos_t dataSize) {
...
static void _maxCodeSize(TLastType &lastType, hpatch_StreamPos_t &curLen0,
                         hpatch_StreamPos_t &curLenv,
                         hpatch_StreamPos_t &fixedLen,
                         const unsigned char *appendData,
                         const unsigned char *appendData_end) {
  while (appendData != appendData_end) {
    unsigned char v;
    size_t sLen0 = 0;
...
hpatch_StreamPos_t
TSingleStreamRLE0::maxCodeSize(const unsigned char *appendData,
  ...
  _maxCodeSize(lastType, curLen0, curLenv, fixedLen, appendData,
               appendData_end);
  _maxCodeSize_end(lastType, curLen0, curLenv, fixedLen);
  return fixedLen;
}

hpatch_StreamPos_t TSingleStreamRLE0::maxCodeSizeByZeroLen(
    hpatch_StreamPos_t appendZeroLen) const {
  hpatch_StreamPos_t fixedLen = this->fixed_code.size();
  if (appendZeroLen == 0)
    return fixedLen;
  TLastType lastType = getLastType(*this);
...
hpatch_StreamPos_t
TSingleStreamRLE0::maxCodeSize(const hpatch_TStreamInput *appendData) const {
  TLastType lastType = getLastType(*this);
  hpatch_StreamPos_t curLen0 = this->len0;
  hpatch_StreamPos_t curLenv = this->uncompressData.size();
  hpatch_StreamPos_t fixedLen = this->fixed_code.size();
  ...
  while (readPos < appendData->streamSize) {
    ...
    if (!appendData->read(appendData, readPos, buf, buf + len))
      throw std::runtime_error(
          "TSingleStreamRLE0::maxCodeSize() appendData->read()");
...
void TSingleStreamRLE0::append(const unsigned char *appendData,
                               const unsigned char *appendData_end) {
  TLastType lastType = getLastType(*this);
  while (appendData != appendData_end) {
    unsigned char v;
...
void TSingleStreamRLE0::append(const hpatch_TStreamInput *appendData) {
  ...
  while (readPos < appendData->streamSize) {
    ...
    if (!appendData->read(appendData, readPos, buf, buf + len))
      throw std::runtime_error(
          "TSingleStreamRLE0::append() appendData->read()");
    append(buf, buf + len);
    readPos += len;
  }
}
