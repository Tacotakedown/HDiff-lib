...
template <class T> inline static void _clearV(std::vector<T> &v) {
  std::vector<T> tmp;
  v.swap(tmp);
}
...
template <bool isNew>
static void _getPackedCovers(hpatch_StreamPos_t dataSize,
                             const std::vector<hpatch_TCover> &blockCovers,
...
struct TOutputCovers : public hpatch_TOutputCovers {
  TOutputCovers(std::vector<hpatch_TCover> &_blockCovers)
      : blockCovers(_blockCovers) {
    blockCovers.clear();
    push_cover = _push_cover;
    collate_covers = _collate_covers;
  }
  static hpatch_BOOL _push_cover(struct hpatch_TOutputCovers *out_covers,
...
void TMatchBlock::getBlockCovers() {
  ...
  get_match_covers_by_block(newData, newData_end, oldData, oldData_end, &covers,
                            matchBlockSize, threadNum);
}

void TMatchBlock::getPackedCover() {
  std::sort(blockCovers.begin(), blockCovers.end(),
            cover_cmp_by_old_t<hpatch_TCover>());
  _getPackedCovers<false>(oldData_end - oldData, blockCovers,
                          packedCoversForOld);
  std::sort(blockCovers.begin(), blockCovers.end(),
            cover_cmp_by_new_t<hpatch_TCover>());
  _getPackedCovers<true>(newData_end - newData, blockCovers,
                         packedCoversForNew);
}

static unsigned char *
doPackData(unsigned char *data, unsigned char *data_end,
...
template <class TCover, bool isNew>
static void _sortCover(TCover *pcovers, size_t coverCount) {
  if (isNew)
...
template <bool isNew>
static void sortCover(void *pcovers, size_t coverCount, bool isCover32) {
  if (coverCount == 0)
    return;
  if (isCover32)
...
template <class TCover, class TPos, bool isNew>
static void _clipCover(TCover *pcovers, size_t coverCount,
                       const std::vector<TPackedCover> &packedCovers,
                       std::vector<hpatch_TCover> &out_clipCovers) {
  if (packedCovers.empty())
    return;
...
template <bool isNew>
static void doClipCover(void *pcovers, size_t coverCount, bool isCover32,
                        const std::vector<TPackedCover> &packedCovers,
                        std::vector<hpatch_TCover> &out_clipCovers) {
  if (coverCount == 0)
    return;
...
#define _insertCovers(icovers)                                                 \
  {                                                                            \
    pcovers = diffi->insertCover(diffi, icovers.data(), icovers.size(),        \
                                 sizeof(*icovers.data()) ==                    \
                                     sizeof(hpatch_TCover32));                 \
    coverCount += icovers.size();                                              \
  }

void TMatchBlock::unpackData(IDiffInsertCover *diffi, void *pcovers,
  ...
  doClipCover<true>(pcovers, coverCount, isCover32, packedCoversForNew,
                    clipCovers);
  ...
  clipCovers.clear();
  doClipCover<false>(pcovers, coverCount, isCover32, packedCoversForOld,
                     clipCovers);
...
void TCoversOptim::_insert_cover(ICoverLinesListener *listener,
                                 IDiffInsertCover *diffi, void *pcovers,
                                 size_t coverCount, bool isCover32,
                                 hpatch_StreamPos_t *newSize,
                                 hpatch_StreamPos_t *oldSize) {
  TCoversOptim *self = (TCoversOptim *)listener;
  if (self->matchBlock != 0) {
    _check((hpatch_StreamPos_t)(self->matchBlock->newData_end_cur -
                                self->matchBlock->newData) == *newSize,
           "unpackData error!");
    _check((hpatch_StreamPos_t)(self->matchBlock->oldData_end_cur -
                                self->matchBlock->oldData) == *oldSize,
           "unpackData error!");
...
void loadOldAndNewStream(TAutoMem &out_mem,
  ...
  if (old_size)
    _check(oldStream->streamSize + newStream->streamSize == old_size + new_size,
           "loadOldAndNew() streamSize");
  out_mem.realloc(old_size + new_size);
  if (old_size)
    _check(oldStream->read(oldStream, 0, out_mem.data(),
                           out_mem.data() + old_size),
           "loadOldAndNew() oldStream->read");
  _check(newStream->read(newStream, 0, out_mem.data() + old_size,
                         out_mem.data() + old_size + new_size),
         "loadOldAndNew() newStream->read");
}
...
void create_compressed_diff_block(
    unsigned char *newData, unsigned char *newData_end, unsigned char *oldData,
    unsigned char *oldData_end, std::vector<unsigned char> &out_diff,
  ...
  if (matchBlockSize == 0) {
    create_compressed_diff(newData, newData_end, oldData, oldData_end, out_diff,
                           compressPlugin, kMinSingleMatchScore,
                           isUseBigCacheMatch, 0, threadNum);
    return;
  }
  TCoversOptimMB<TMatchBlock> coversOp(newData, newData_end, oldData,
                                       oldData_end, matchBlockSize, threadNum);
  create_compressed_diff(newData, coversOp.matchBlock->newData_end_cur, oldData,
                         coversOp.matchBlock->oldData_end_cur, out_diff,
                         compressPlugin, kMinSingleMatchScore,
                         isUseBigCacheMatch, &coversOp, threadNum);
}
void create_compressed_diff_block(
    unsigned char *newData, unsigned char *newData_end, unsigned char *oldData,
    unsigned char *oldData_end, const hpatch_TStreamOutput *out_diff,
    const hdiff_TCompress *compressPlugin, int kMinSingleMatchScore,
    bool isUseBigCacheMatch, size_t matchBlockSize, size_t threadNum) {
  if (matchBlockSize == 0) {
    create_compressed_diff(newData, newData_end, oldData, oldData_end, out_diff,
...
void create_compressed_diff_block(const hpatch_TStreamInput *newData,
  ...
  create_compressed_diff_block(pNewData, pNewData + (size_t)newData->streamSize,
                               pOldData, pOldData + old_size, out_diff,
                               compressPlugin, kMinSingleMatchScore,
                               isUseBigCacheMatch, matchBlockSize, threadNum);
}

void create_single_compressed_diff_block(
    unsigned char *newData, unsigned char *newData_end, unsigned char *oldData,
    unsigned char *oldData_end, const hpatch_TStreamOutput *out_diff,
    const hdiff_TCompress *compressPlugin, int kMinSingleMatchScore,
    size_t patchStepMemSize, bool isUseBigCacheMatch, size_t matchBlockSize,
    size_t threadNum) {
  if (matchBlockSize == 0) {
    create_single_compressed_diff(newData, newData_end, oldData, oldData_end,
                                  out_diff, compressPlugin,
  ...
  TCoversOptimMB<TMatchBlock> coversOp(newData, newData_end, oldData,
                                       oldData_end, matchBlockSize, threadNum);
  create_single_compressed_diff(newData, coversOp.matchBlock->newData_end_cur,
                                oldData, coversOp.matchBlock->oldData_end_cur,
...
void create_single_compressed_diff_block(
    ...
    int kMinSingleMatchScore, size_t patchStepMemSize, bool isUseBigCacheMatch,
    size_t matchBlockSize, size_t threadNum) {
  TAutoMem oldAndNewData;
  loadOldAndNewStream(oldAndNewData, oldData, newData);
  size_t old_size = oldData ? (size_t)oldData->streamSize : 0;
  ...
  create_single_compressed_diff_block(
      pNewData, pNewData + (size_t)newData->streamSize, pOldData,
      pOldData + old_size, out_diff, compressPlugin, kMinSingleMatchScore,
