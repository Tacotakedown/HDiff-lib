...
size_t
_getRegionRleCost(const unsigned char *d, size_t n, const unsigned char *sub,
  ...
  while (i < n) {
    ...
    ++i;
    while ((i < n) &&
           (v == (is_sub_on ? (unsigned char)(d[i] - sub[i]) : d[i])))
      ++i;
    if ((v == 0) | (v == 255)) {
      ++cost;
    } else {
...
size_t getRegionRleCost(const unsigned char *d, size_t n,
                        const unsigned char *sub, unsigned char *out_nocompress,
                        size_t *nocompressSize) {
  assert((nocompressSize == 0) || (*nocompressSize >= n));
  if (sub) {
    if (out_nocompress)
      return _getRegionRleCost<true, true>(d, n, sub, out_nocompress,
                                           nocompressSize);
    else
      return _getRegionRleCost<true, false>(d, n, sub, 0, nocompressSize);
  } else {
    if (out_nocompress)
      return _getRegionRleCost<false, true>(d, n, 0, out_nocompress,
                                            nocompressSize);
    else
...
size_t TCompressDetect::_cost_rle(const unsigned char *d, size_t n) const {
  ...
  return (codeSize + ((kCompressDetectDivBit >> 1) + 1)) /
...
void TCompressDetect::add_chars(const unsigned char *d, size_t n,
...
size_t TCompressDetect::cost(const unsigned char *d, size_t n,
                             const unsigned char *sub) const {
  size_t result = 0;
