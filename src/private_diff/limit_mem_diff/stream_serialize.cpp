...
struct TCompressedStream : public hpatch_TStreamOutput {
  TCompressedStream(const hpatch_TStreamOutput *_out_code,
                    ...
                    hpatch_StreamPos_t _kLimitOutCodeSize);
...
private:
  ...
  bool _is_overLimit;
  static hpatch_BOOL _write_code(const hpatch_TStreamOutput *stream,
...
TCompressedStream::TCompressedStream(const hpatch_TStreamOutput *_out_code,
                                     hpatch_StreamPos_t _writePos,
                                     hpatch_StreamPos_t _kLimitOutCodeSize)
    : out_code(_out_code), out_pos(_writePos),
      out_posLimitEnd(_writePos + _kLimitOutCodeSize), _writeToPos_back(0),
      _is_overLimit(false) {
...
hpatch_BOOL TCompressedStream::_write_code(const hpatch_TStreamOutput *stream,
                                           hpatch_StreamPos_t writeToPos,
                                           const unsigned char *data,
                                           const unsigned char *data_end) {
  assert(data < data_end);
  TCompressedStream *self = (TCompressedStream *)stream->streamImport;
  checki(self->_writeToPos_back == writeToPos,
         "TCompressedStream::write() writeToPos error!");
  size_t dataLen = (size_t)(data_end - data);
  self->_writeToPos_back = writeToPos + dataLen;

  if ((self->_is_overLimit) ||
      (self->out_pos + dataLen > self->out_posLimitEnd)) {
    self->_is_overLimit = true;
  } else {
    if (!self->out_code->write(self->out_code, self->out_pos, data, data_end))
      return hpatch_FALSE;
...
TCoversStream::TCoversStream(const TCovers &_covers,
                             hpatch_StreamPos_t cover_buf_size)
    : covers(_covers), curCodePos(0), curCodePos_end(0), readedCoverCount(0),
      lastOldEnd(0), lastNewEnd(0), _readFromPos_back(0) {
...
#define __private_packCover(_TUInt, packWithTag, pack, dst, dst_end, cover,    \
                            lastOldEnd, lastNewEnd)                            \
  {                                                                            \
    if (cover.oldPos >= lastOldEnd) /*save inc_oldPos*/                        \
      packWithTag(dst, dst_end, (_TUInt)(cover.oldPos - lastOldEnd), 0, 1);    \
    else                                                                       \
      packWithTag(dst, dst_end, (_TUInt)(lastOldEnd - cover.oldPos), 1,        \
                  1); /*sub safe*/                                             \
    pack(dst, dst_end,                                                         \
         (_TUInt)(cover.newPos - lastNewEnd)); /*save inc_newPos*/             \
    pack(dst, dst_end, (_TUInt)cover.length);                                  \
  }

hpatch_BOOL TCoversStream::_read(const hpatch_TStreamInput *stream,
  ...
  if (readFromPos == 0) {
    ...
    checki(self->_readFromPos_back == readFromPos,
           "TCoversStream::read() readFromPos error!");
  }
  ...
  while (out_data < out_data_end) {
    ...
    if (curLen > 0) {
      ...
      for (; ((size_t)(pcode_end - pcode_cur) >=
              hpatch_kMaxPackedUIntBytes * 3) &&
             (cur_index < n);
           ++cur_index) {
        TCover cover;
        self->covers.covers(cur_index, &cover);
        __private_packCover(hpatch_StreamPos_t, hpatch_packUIntWithTag,
                            hpatch_packUInt, &pcode_cur, pcode_end, cover,
                            self->lastOldEnd, self->lastNewEnd);
...
inline static void _packUIntWithTag_size(hpatch_StreamPos_t *curSize, int _,
                                         hpatch_StreamPos_t uValue, int highTag,
                                         const int kTagBit) {
  *curSize += hpatch_packUIntWithTag_size(uValue, kTagBit);
}
inline static void _packUInt_size(hpatch_StreamPos_t *curSize, int _,
...
hpatch_StreamPos_t TCoversStream::getDataSize(const TCovers &covers) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    __private_packCover(hpatch_StreamPos_t, _packUIntWithTag_size,
                        _packUInt_size, &cover_buf_size, 0, cover, lastOldEnd,
                        lastNewEnd);
...
void TNewDataDiffStream::_initByRange(size_t coveri,
                                      hpatch_StreamPos_t newDataPos,
                                      hpatch_StreamPos_t newDataPosEnd) {
...
hpatch_BOOL TNewDataDiffStream::_read(const hpatch_TStreamInput *stream,
  ...
  while (out_data < out_data_end) {
    ...
    if (curLen > 0) {
      ...
      if (!self->newData->read(self->newData, self->curNewPos, out_data,
                               out_data + readLen))
        return hpatch_FALSE;
      out_data += readLen;
...
hpatch_StreamPos_t
TNewDataDiffStream::getDataSizeByRange(const TCovers &covers, size_t coveri,
                                       hpatch_StreamPos_t newDataPos,
                                       hpatch_StreamPos_t newDataPosEnd) {
  size_t n = covers.coverCount();
...
hpatch_StreamPos_t
TNewDataDiffStream::getDataSize(const TCovers &covers,
...
TNewDataSubDiffStream::TNewDataSubDiffStream(const hdiff_TStreamInput *_newData,
                                             const hdiff_TStreamInput *_oldData,
                                             const TCovers &_covers,
                                             bool _isOnlySubCover,
                                             bool _isZeroSubDiff)
    : newData(_newData), oldData(_oldData), covers(_covers),
      isOnlySubCover(_isOnlySubCover), isZeroSubDiff(_isZeroSubDiff),
      _cache(kSubDiffCacheSize) {
  initRead();
  streamImport = this;
  if (isOnlySubCover) {
    streamSize = 0;
...
static inline void _subData(unsigned char *dst, const unsigned char *src,
                            size_t length) {
  while (length--)
    (*dst++) -= *src++;
}
static void _subData(unsigned char *out_data, size_t len,
                     ...
                     hpatch_StreamPos_t oldPos, unsigned char *cache,
                     size_t cacheSize) {
  while (len) {
    size_t rlen = len;
    if (rlen > cacheSize)
...
void TNewDataSubDiffStream::readTo(unsigned char *out_data,
                                   unsigned char *out_data_end) {
  size_t readLen = out_data_end - out_data;
  while (readLen > 0) {
    if (curDataLen == 0) {
    ...
    if ((!isZeroSubDiff) && (curReadOldPos != _kNullPos)) {
      ...
      _subData(out_data, len, oldData, curReadOldPos, _cache.data(),
               _cache.size());
    } else {
      memset(out_data, 0, len);
    }
    if (curReadOldPos != _kNullPos)
      curReadOldPos += len;
    curReadNewPos += len;
    curDataLen -= len;
    out_data += len;
...
hpatch_BOOL TNewDataSubDiffStream::_read(
    const struct hpatch_TStreamInput *stream, hpatch_StreamPos_t readFromPos,
...
TNewDataSubDiffStream_mem::TNewDataSubDiffStream_mem(
    ...
    : TNewDataSubDiffStream(
          mem_as_hStreamInput(&mem_newData, newData, newData_end),
          mem_as_hStreamInput(&mem_oldData, oldData, oldData_end), _covers,
          _isOnlySubCover, _isZeroSubDiff) {}

TNewDataSubDiffCoverStream::TNewDataSubDiffCoverStream(
    const hpatch_TStreamInput *_newStream,
    const hpatch_TStreamInput *_oldStream, bool _isZeroSubDiff)
    : isZeroSubDiff(_isZeroSubDiff), newStream(_newStream),
      oldStream(_oldStream), newData(0), oldData(0),
      _cache(_isZeroSubDiff ? 0 : kSubDiffCacheSize * 2) {
...
hpatch_BOOL TNewDataSubDiffCoverStream::_read(const hpatch_TStreamInput *stream,
                                              hpatch_StreamPos_t readFromPos,
                                              unsigned char *out_data,
                                              unsigned char *out_data_end) {
  TNewDataSubDiffCoverStream *self =
      (TNewDataSubDiffCoverStream *)stream->streamImport;
...
hpatch_BOOL TNewDataSubDiffCoverStream::readTo(hpatch_StreamPos_t readFromPos,
  ...
  while (out_data != out_data_end) {
    ...
    if ((readFromPos < skipLen) || (readFromPos >= (skipLen + curDataLen))) {
      if (!_updateCache(readFromPos))
        return hpatch_FALSE;
      skipLen = cover.length - (curDataLen + inStreamLen);
    }
    size_t srcPos = (size_t)(readFromPos - skipLen);
    size_t len = std::min((size_t)(curDataLen - srcPos),
                          (size_t)(out_data_end - out_data));
...
hpatch_BOOL
TNewDataSubDiffCoverStream::_updateCache(hpatch_StreamPos_t readFromPos) {
  ...
  if (!newStream->read(newStream, cover.newPos + readFromPos, newData,
                       newData + curDataLen))
    return hpatch_FALSE;
  if (!oldStream->read(oldStream, cover.oldPos + readFromPos, oldData,
                       oldData + curDataLen))
    return hpatch_FALSE;
  return hpatch_TRUE;
}

TStepStream::TStepStream(const hpatch_TStreamInput *newStream,
                         const hpatch_TStreamInput *oldStream,
                         bool isZeroSubDiff, const TCovers &_covers,
                         size_t _patchStepMemSize)
    : subDiff(newStream, oldStream, isZeroSubDiff),
      newDataDiff(_covers, newStream), covers(_covers),
      patchStepMemSize(_patchStepMemSize), newDataSize(newStream->streamSize),
...
hpatch_BOOL TStepStream::readTo(unsigned char *out_data,
                                unsigned char *out_data_end) {
  while (out_data != out_data_end) {
...
hpatch_BOOL TStepStream::_read(const hpatch_TStreamInput *stream,
                               hpatch_StreamPos_t readFromPos,
                               unsigned char *out_data,
                               unsigned char *out_data_end) {
...
bool TStepStream::doStep() {
  if (pCurCover == 0) {
    if (isHaveLeftCover) {
      isHaveLeftCover = false;
      pCurCover = &leftCover;
    } else if (isHaveRightCover) {
      isHaveRightCover = false;
      pCurCover = &rightCover;
    } else if (cur_i < covers.coverCount()) {
      covers.covers(cur_i++, &cover);
      pCurCover = &cover;
    } else if (isHaveLastCover) {
      isHaveLastCover = false;
      pCurCover = &lastCover;
    } else if (step_bufCover_size > 0) {
      _last_flush_step();
      return true;
    } else {
      return false;
  ...
  if (cover.oldPos >= lastOldEnd) { // save inc_oldPos
    if (isInInit)
      step_bufCover_size += hpatch_packUIntWithTag_size(
          (hpatch_StreamPos_t)(cover.oldPos - lastOldEnd), 1);
    else
      packUIntWithTag(step_bufCover,
                      (hpatch_StreamPos_t)(cover.oldPos - lastOldEnd), 0, 1);
  } else {
    if (isInInit)
      step_bufCover_size += hpatch_packUIntWithTag_size(
          (hpatch_StreamPos_t)(lastOldEnd - cover.oldPos), 1);
    else
      packUIntWithTag(step_bufCover,
                      (hpatch_StreamPos_t)(lastOldEnd - cover.oldPos), 1,
                      1); // sub safe
  ...
  if (isInInit) {
    step_bufCover_size +=
        hpatch_packUInt_size(backNewLen) + hpatch_packUInt_size(cover.length);
  } else {
  ...
  const hpatch_StreamPos_t curMaxNeedSize =
      step_bufCover_size +
      (subDiff.isZeroSubDiff
           ? step_bufRle.maxCodeSizeByZeroLen(subDiff.streamSize)
           : step_bufRle.maxCodeSize(&subDiff));
  if (curMaxNeedSize <= patchStepMemSize) { // append
    ...
    if (step_bufCover_backSize + step_bufRle.curCodeSize() >=
        (patchStepMemSize / 2)) { // flush step
      step_bufCover_size = step_bufCover_backSize;
      if (!isInInit)
        step_bufCover.resize(step_bufCover_backSize);

      step_dataDiff.reset(&newDataDiff, newDataDiffReadPos - step_dataDiffSize,
                          newDataDiffReadPos);
      step_dataDiffSize = 0;
      _flush_step_code();
      ...
      while (1) {
        ...
        const hpatch_StreamPos_t _curMaxNeedSize =
            step_bufCover_size +
            (subDiff.isZeroSubDiff
                 ? step_bufRle.maxCodeSizeByZeroLen(subDiff.streamSize)
                 : step_bufRle.maxCodeSize(&subDiff));
...
void TStepStream::_flush_step_code() {
  ...
  const size_t curStepMemSize =
      step_bufCover_size + step_bufRle.fixed_code.size();
...
size_t TDiffStream::packUInt(hpatch_StreamPos_t uValue) {
  ...
  check(
      hpatch_packUInt(&codeEnd, codeBuf + hpatch_kMaxPackedUIntBytes, uValue));
  pushBack(codeBuf, (size_t)(codeEnd - codeBuf));
  return codeEnd - codeBuf;
}

void TDiffStream::_packUInt_limit(hpatch_StreamPos_t uValue,
                                  size_t limitOutSize) {
...
void TDiffStream::packUInt_update(const TPlaceholder &pos,
                                  hpatch_StreamPos_t uValue) {
...
void TDiffStream::stream_read(const TPlaceholder &pos, hpatch_byte *out_data) {
  ...
  checki(out_diff->read_writed != 0,
         "TDiffStream::stream_read() out_diff can't read error!");
  checki(out_diff->read_writed(out_diff, pos.pos, out_data,
                               out_data + (size_t)pos.size()),
         "TDiffStream::stream_read() out_diff read error!");
}
void TDiffStream::stream_update(const TPlaceholder &pos,
                                const hpatch_byte *in_data) {
  assert(pos.pos <= pos.pos_end);
  assert(pos.size() == (size_t)pos.size());
  check(pos.pos_end <= writePos);
...
hpatch_StreamPos_t TDiffStream::pushStream(
    const hpatch_TStreamInput *stream, const hdiff_TCompress *compressPlugin,
    const TPlaceholder &update_compress_sizePos, bool isMustCompress,
    const hpatch_StreamPos_t cancelSizeOnCancelCompress) {
  ...
  if ((compressPlugin) &&
    ...
    hpatch_StreamPos_t kLimitOutCodeSize =
        isMustCompress
            ? compressPlugin->maxCompressedSize(stream->streamSize + 1)
            : (stream->streamSize - (1 + cancelSizeOnCancelCompress));
    TCompressedStream out_stream(out_diff, writePos, kLimitOutCodeSize);
    compressed_size =
        compressPlugin->compress(compressPlugin, &out_stream, stream);
    if (out_stream.is_overLimit() || (compressed_size == 0) ||
        (compressed_size > kLimitOutCodeSize)) {
...
void TStreamClip::reset(const hpatch_TStreamInput *stream,
                        ...
                        hpatch_StreamPos_t uncompressSize) {
  clear();
...
void TStreamClip::openDecompressHandle() {
  ...
  _decompressHandle = _decompressPlugin->open(
      _decompressPlugin, this->streamSize, _src, _src_begin, _src_end);
  check(_decompressHandle != 0);
}

hpatch_BOOL TStreamClip::_clip_read(const hpatch_TStreamInput *stream,
                                    hpatch_StreamPos_t readFromPos,
                                    unsigned char *out_data,
                                    unsigned char *out_data_end) {
  ...
  if (self->_decompressPlugin) {
    return self->_decompressPlugin->decompress_part(self->_decompressHandle,
                                                    out_data, out_data_end);
  } else {
    return self->_src->read(self->_src, self->_src_begin + readFromPos,
                            out_data, out_data_end);
  }
}

_TCheckOutNewDataStream::_TCheckOutNewDataStream(
    const hpatch_TStreamInput *_newData, unsigned char *_buf, size_t _bufSize)
...
hpatch_BOOL _TCheckOutNewDataStream::_read_writed(
    const struct hpatch_TStreamOutput *stream, hpatch_StreamPos_t readFromPos,
    unsigned char *out_data, unsigned char *out_data_end) {
  _TCheckOutNewDataStream *self =
      (_TCheckOutNewDataStream *)stream->streamImport;
  _test_rt(readFromPos <= self->writedLen);
  _test_rt((hpatch_size_t)(out_data_end - out_data) <=
           (hpatch_StreamPos_t)(self->writedLen - readFromPos));
  return self->newData->read(self->newData, readFromPos, out_data,
                             out_data_end);
}
hpatch_BOOL _TCheckOutNewDataStream::_write_check(
    const hpatch_TStreamOutput *stream, hpatch_StreamPos_t writeToPos,
  ...
  while (data < data_end) {
    ...
    if (readLen > self->bufSize)
      readLen = self->bufSize;
    _test_rt(self->newData->read(self->newData, readPos, self->buf,
                                 self->buf + readLen));
...
void do_compress(std::vector<unsigned char> &out_code,
  ...
  if (data->streamSize == 0)
    return;
  hpatch_StreamPos_t maxCodeSize =
      compressPlugin->maxCompressedSize(data->streamSize);
  if ((maxCodeSize <= data->streamSize) || (maxCodeSize != (size_t)maxCodeSize))
    return; // error
  out_code.resize((size_t)maxCodeSize);
  hpatch_TStreamOutput codeStream;
  mem_as_hStreamOutput(&codeStream, out_code.data(),
                       out_code.data() + out_code.size());
  hpatch_StreamPos_t codeSize =
      compressPlugin->compress(compressPlugin, &codeStream, data);
