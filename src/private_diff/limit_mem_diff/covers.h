...
static inline void setCover(TCover &cover, hpatch_StreamPos_t oldPos,
...
static void tm_collate_covers(std::vector<_TCover> &covers) {
  ...
  for (size_t i = 1; i < covers.size(); ++i) {
    if (covers[i].newPos < covers[backi].newPos + covers[backi].length) {
      if (covers[i].newPos + covers[i].length >
          covers[backi].newPos + covers[backi].length) {
        if (cover_is_collinear(covers[i],
                               covers[backi])) { // insert i part to backi,del i
          covers[backi].length =
              covers[i].newPos + covers[i].length - covers[backi].newPos;
    ...
    } else if ((covers[i].newPos ==
                covers[backi].newPos + covers[backi].length) &&
               (covers[i].oldPos ==
                covers[backi].oldPos + covers[backi].length)) {
...
private:
  template <class _TCover> inline void _update(std::vector<_TCover> &covers) {
    _covers = covers.data();
