...
struct CAutoLocker : public _TLockerBox_name {
  inline CAutoLocker(HLocker _locker) : _TLockerBox_name() {
    if (_locker) {
      _TLockerBox_name _t(*(std::mutex *)_locker);
      _t.swap(*this);
    }
  }
  inline ~CAutoLocker() {}
...
struct TMtByChannel {
  ...
  bool start_threads(int threadCount, TThreadRunCallBackProc threadProc,
                     void *workData, bool isUseThisThread) {
  ...
  inline void thread_end() { _end_chan.send((TChanData)1, true); }
...
private:
  inline void thread_on(int threadNum) { _is_thread_on += threadNum; }
