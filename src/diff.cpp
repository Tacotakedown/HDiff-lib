...
static const int kMinMatchLen = (_SSTRING_FAST_MATCH > kCoverMinMatchLen)
                                    ? _SSTRING_FAST_MATCH
                                    : kCoverMinMatchLen;
#else
...
namespace
{
  ...
  static const int kMaxLinkSpaceLength =
      (1 << 9) - 1; // 跨覆盖线合并时,允许合并的最远距离.
  ...
  struct TOldCover
  {
    ...
    inline bool
    isCollinear(const TOldCover &next) const
    { // 覆盖线是否在同一条直线上.
...
#define getEqualLength(x, x_end, y, y_end) \
  getEqualLengthLimit<0>(x, x_end, y, y_end)
                           ...
                           size_t *out_limitSkip = 0)
  {
    ...
    const TInt kLimitOldEnd =
        (TInt)(diffLimit ? diffLimit->recoverOldEnd : sstring.SASize());
...
#define kTryEqLenLimit (1024 * 1 + 17)
      ...
      {
        curLength =
            getEqualLength(newData, newData_end, src_begin + curOldPos, src_end);
      }
      else
      {
        curLength = getEqualLengthLimit<kTryEqLenLimit>(
            newData, newData_end, src_begin + curOldPos, src_end);
      ...
      if (curLength > bestLength)
      {
        if (diffLimit)
        {
          hpatch_TCover cover = {(size_t)curOldPos, (size_t)curNewPos,
                                 (size_t)curLength};
          ...
          {
            curLength = getEqualLength(newData, newData_end,
                                       src_begin + curOldPos, src_end);
            hpatch_TCover cover = {(size_t)curOldPos, (size_t)curNewPos,
                                   (size_t)curLength};
            diffLimit->listener->limitCover(diffLimit->listener, &cover, &hitPos);
          }
  ...
  // 粗略估算覆盖线的控制数据成本;
  inline static TInt getCoverCtrlCost(const TOldCover &cover,
                                      const TOldCover &lastCover)
  {
    ...
    return _getIntCost<TInt, TUInt>((TInt)(cover.oldPos - lastCover.oldPos)) +
           ...
           _getUIntCost((TUInt)(cover.newPos - lastCover.newPos)) +
           kUnLinkOtherScore;
  }
  ...
  {
    return (TInt)getRegionRleCost(diff.newData + cover.newPos, cover.length,
                                  diff.oldData + cover.oldPos);
  }

  // 尝试延长lastCover来完全代替matchCover;
  static bool tryLinkExtend(TOldCover &lastCover, const TOldCover &matchCover,
                            const TDiffData &diff, TDiffLimit *diffLimit)
  {
    if (lastCover.length <= 0)
      return false;
    const TInt linkSpaceLength =
        (matchCover.newPos - (lastCover.newPos + lastCover.length));
    ...
    if (diffLimit)
    {
      ...
      hpatch_TCover cover = {
          (size_t)(lastCover.oldPos + lastCover.length), cnewPos,
    ...
    TInt lastLinkCost =
        (TInt)getRegionRleCost(diff.newData + matchCover.newPos,
                               matchCover.length, diff.oldData + linkOldPos);
    if (lastLinkCost > matchCost)
      return false;
    TInt len =
        lastCover.length + linkSpaceLength +
        (matchCover.length * 2 / 3); // 扩展大部分,剩下的可能扩展留给extend_cover.
    len +=
        getEqualLength(diff.newData + lastCover.newPos + len, diff.newData_end,
    ...
    if (diffLimit)
    {
      ...
      if (len > safeLen)
      {
        hpatch_TCover cover = {(size_t)(lastCover.oldPos + safeLen),
                               (size_t)(lastCover.newPos + safeLen),
                               (size_t)(len - safeLen)};
        hpatch_StreamPos_t hitPos;
        diffLimit->listener->limitCover(diffLimit->listener, &cover, &hitPos);
  ...
  static void tryCollinear(TOldCover &lastCover, const TOldCover &matchCover,
                           const TDiffData &diff, TDiffLimit *diffLimit)
  {
    if (lastCover.length <= 0)
      return;
    if (lastCover.isCollinear(matchCover))
      return; // 已经共线;

    TInt linkOldPos = matchCover.oldPos - (matchCover.newPos - lastCover.newPos);
    if ((linkOldPos < 0) ||
        (linkOldPos + lastCover.length > (diff.oldData_end - diff.oldData)))
      return;
    if (diffLimit)
    {
      hpatch_TCover cover = {(size_t)linkOldPos, (size_t)lastCover.newPos,
                             (size_t)lastCover.length};
      if (!diffLimit->listener->limitCover(diffLimit->listener, &cover, 0))
        return;
    }
    TInt lastCost = getCoverCost(lastCover, diff);
    TInt matchLinkCost =
        (TInt)getRegionRleCost(diff.newData + lastCover.newPos, lastCover.length,
                               diff.oldData + linkOldPos);
  ...
  static void _search_cover(std::vector<TOldCover> &covers, const TDiffData &diff,
                            const TSuffixString &sstring, TDiffLimit *diffLimit,
                            bool isCanExtendCover)
  {
    ...
    const TInt newEnd =
        diffLimit ? diffLimit->newEnd : (diff.newData_end - diff.newData);
    if (newEnd - newPos <= kMinMatchLen)
      return;
    ...
    while (newPos <= maxSearchNewPos)
    {
      ...
      size_t limitSkip = 1;
      TInt matchEqLength =
          getBestMatch(&matchOldPos, sstring, diff.newData + newPos,
                       diff.newData + newEnd, newPos, diffLimit, &limitSkip);
      ...
      if (matchEqLength - getCoverCtrlCost(matchCover, lastCover) <
          kMinMatchScore)
      {
      ...
      if (isCanExtendCover)
      {
        if (tryLinkExtend(lastCover, matchCover, diff, diffLimit))
        { // use link
      ...
      newPos = std::max(
          newPos + 1,
          lastCover.newPos +
              lastCover.length); // 选出的cover不允许重叠,这可能不是最优策略;
    }
  }

  // 选择合适的覆盖线,去掉不合适的.
  static void _select_cover(std::vector<TOldCover> &covers, size_t cover_begin,
                            const TDiffData &diff, int kMinSingleMatchScore,
                            TCompressDetect &nocover_detect,
                            TCompressDetect &cover_detect, TDiffLimit *diffLimit,
                            bool isCanExtendCover)
  {
    TOldCover lastCover(0, 0, 0);
    if (diffLimit)
    ...
    for (size_t i = cover_begin; i < coverSize_old; ++i)
    {
      ...
      { // 向前合并可能.
        if ((insertIndex > cover_begin) &&
            (covers[insertIndex - 1].isCanLink(covers[i])))
        {
          if (diffLimit)
          {
            const TOldCover &fc = covers[insertIndex - 1];
            hpatch_TCover cover = {(size_t)(fc.oldPos + fc.length),
                                   (size_t)(fc.newPos + fc.length),
      ...
      { // 查询向后合并可能link
        ...
        {
          if (!covers[i].isCanLink(covers[j]))
            break;
          if (diffLimit)
          {
      ...
      if (!isNeedSave)
      { // 单覆盖是否保留.
        TInt noCoverCost = nocover_detect.cost(diff.newData + covers[i].newPos,
                                               covers[i].length);
        TInt coverCost =
            cover_detect.cost(diff.newData + covers[i].newPos, covers[i].length,
                              diff.oldData + covers[i].oldPos);
        TInt coverSorce =
            noCoverCost - coverCost - getCoverCtrlCost(covers[i], lastCover);
      ...
      if (isNeedSave)
      {
        ...
        { // link合并.
          ...
          cover_detect.add_chars(
              diff.newData + lastCover.newPos + lastCover.length,
        ...
        else
        {
          ...
          nocover_detect.add_chars(
              diff.newData + lastCover.newPos + lastCover.length,
              covers[i].newPos - (lastCover.newPos + lastCover.length));
          cover_detect.add_chars(diff.newData + covers[i].newPos,
                                 covers[i].length,
                                 diff.oldData + covers[i].oldPos);
        }
  ...
  static void select_cover(std::vector<TOldCover> &covers, size_t cover_begin,
                           const TDiffData &diff, int kMinSingleMatchScore,
                           TDiffLimit *diffLimit, bool isCanExtendCover)
  {
    if (diffLimit == 0)
    {
      TCompressDetect nocover_detect;
      TCompressDetect cover_detect;
      _select_cover(covers, cover_begin, diff, kMinSingleMatchScore,
                    nocover_detect, cover_detect, 0, isCanExtendCover);
    }
    else
    {
      _select_cover(covers, cover_begin, diff, kMinSingleMatchScore,
                    diffLimit->nocover_detect, diffLimit->cover_detect, diffLimit,
                    isCanExtendCover);
    }
  }

  typedef size_t TFixedFloatSmooth; // 定点数.
  static const TFixedFloatSmooth kFixedFloatSmooth_base =
      1024; // 定点数小数点位置.

  // 得到可以扩展位置的长度.
  static TInt getCanExtendLength(TInt oldPos, TInt newPos, int inc,
                                 TInt newPos_min, TInt lastNewEnd,
                                 const TDiffData &diff,
                                 const TFixedFloatSmooth kExtendMinSameRatio)
  {
    ...
    const TFixedFloatSmooth kLimitSameCount =
        (~(TFixedFloatSmooth)0) / kFixedFloatSmooth_base;
    for (TUInt length = 1;
         (oldPos >= 0) && (oldPos < (diff.oldData_end - diff.oldData)) &&
         (newPos >= newPos_min) && (newPos < lastNewEnd);
         ++length, oldPos += inc, newPos += inc)
    {
      if (diff.oldData[oldPos] == diff.newData[newPos])
      {
        ++curSameCount;

        if (curSameCount >= kLimitSameCount)
          break; // for curSameCount*kFixedFloatSmooth_base
        const TFixedFloatSmooth curSameRatio =
            (curSameCount * kFixedFloatSmooth_base) / (length + kSmoothLength);
  ...
  // 尝试延长覆盖区域.
  static void extend_cover(std::vector<TOldCover> &covers, size_t cover_begin,
                           const TDiffData &diff,
                           const TFixedFloatSmooth kExtendMinSameRatio,
                           TDiffLimit *diffLimit = 0)
  {
    ...
    for (size_t i = cover_begin; i < covers.size(); ++i)
    {
      ...
      if (i + 1 < covers.size())
        ...
        newPos_next = diffLimit ? diffLimit->newEnd
                                : (TInt)(diff.newData_end - diff.newData);
      TOldCover &curCover = covers[i];
      if (diffLimit)
      {
        TInt limit_front =
            std::min(curCover.newPos - lastNewEnd, curCover.oldPos);
        if (limit_front > 0)
        {
          hpatch_TCover cover = {(size_t)(curCover.oldPos - limit_front),
                                 (size_t)(curCover.newPos - limit_front),
                                 (size_t)limit_front};
          hpatch_StreamPos_t lenLimit;
          diffLimit->listener->limitCover_front(diffLimit->listener, &cover,
                                                &lenLimit);
          lastNewEnd = curCover.newPos - (TInt)lenLimit;
        }

        TInt limit_back = newPos_next - (curCover.newPos + curCover.length);
        if ((curCover.oldPos + curCover.length) + limit_back >
            (diff.oldData_end - diff.oldData))
          limit_back = (diff.oldData_end - diff.oldData) -
                       (curCover.oldPos + curCover.length);
        if (limit_back > 0)
        {
          hpatch_TCover cover = {(size_t)(curCover.oldPos + curCover.length),
                                 (size_t)(curCover.newPos + curCover.length),
                                 (size_t)limit_back};
          hpatch_StreamPos_t lenLimit;
      ...
      // 向前延伸.
      TInt extendLength_front =
          getCanExtendLength(curCover.oldPos - 1, curCover.newPos - 1, -1,
                             lastNewEnd, newPos_next, diff, kExtendMinSameRatio);
      ...
      // 向后延伸.
      TInt extendLength_back = getCanExtendLength(
          curCover.oldPos + curCover.length, curCover.newPos + curCover.length, 1,
          lastNewEnd, newPos_next, diff, kExtendMinSameRatio);
  ...
  static void assert_cover_safe(const _TCover &cover, _TInt lastNewEnd,
                                _TInt newSize, _TInt oldSize)
  {
  ...
  static void assert_covers_safe(const TCovers &covers,
  ...
  static void assert_covers_safe(const std::vector<TOldCover> &covers,
                                 hpatch_StreamPos_t newSize,
                                 hpatch_StreamPos_t oldSize)
  {
  ...
  static void serialize_diff(const TDiffData &diff,
                             const std::vector<TOldCover> &covers,
                             std::vector<TByte> &out_diff)
  {
    ...
    std::vector<TByte> inc_oldPos_buf;
    {
      ...
      for (TUInt i = 0; i < coverCount; ++i)
      {
        ...
        packUInt(inc_newPos_buf,
                 (TUInt)(covers[i].newPos - lastNewEnd)); // save inc_newPos
        if (covers[i].oldPos >= oldPosBack)
        { // save inc_oldPos
          packUIntWithTag(inc_oldPos_buf, (TUInt)(covers[i].oldPos - oldPosBack),
                          0, 1);
        }
        else
        {
          packUIntWithTag(inc_oldPos_buf, (TUInt)(oldPosBack - covers[i].oldPos),
                          1, 1); // sub safe
        }
    ...
    TNewDataSubDiffStream_mem newDataSubDiff(
        diff.newData, diff.newData_end, diff.oldData, diff.oldData_end, _covers);
  ...
  static void _outType(std::vector<TByte> &out_data, T *compressPlugin,
                       const char *versionType = kHDiffVersionType)
  {
    // type version
                                        ...
                                        const hdiff_TCompress *compressPlugin)
  {
    ...
    std::vector<TByte> cover_buf;
    {
      ...
      for (TUInt i = 0; i < coverCount; ++i)
      {
        if (covers[i].oldPos >= lastOldEnd)
        { // save inc_oldPos
          packUIntWithTag(cover_buf, (TUInt)(covers[i].oldPos - lastOldEnd), 0,
                          1);
        }
        else
        {
          packUIntWithTag(cover_buf, (TUInt)(lastOldEnd - covers[i].oldPos), 1,
                          1); // sub safe
        }
        assert(covers[i].newPos >= lastNewEnd);
        packUInt(cover_buf,
  ...
  static void _dispose_cover(std::vector<TOldCover> &covers, size_t cover_begin,
                             const TDiffData &diff, int kMinSingleMatchScore,
                             TDiffLimit *diffLimit, bool isCanExtendCover)
  {
    if (isCanExtendCover)
    {
      ...
      if (kExtendMinSameRatio < 200)
        kExtendMinSameRatio = 200;
      if (kExtendMinSameRatio > 800)
        kExtendMinSameRatio = 800;

      extend_cover(covers, cover_begin, diff, kExtendMinSameRatio,
                   diffLimit); // 先尝试扩展.
      select_cover(covers, cover_begin, diff, kMinSingleMatchScore, diffLimit,
                   isCanExtendCover);
      extend_cover(covers, cover_begin, diff, kExtendMinSameRatio,
                   diffLimit); // select_cover会删除一些覆盖线,所以重新扩展.
    }
    else
    {
      select_cover(covers, cover_begin, diff, kMinSingleMatchScore, diffLimit,
  ...
  static void
  search_and_dispose_cover(std::vector<TOldCover> &covers, const TDiffData &diff,
...
#define first_search_and_dispose_cover(covers, diff, sstring,                  \
                                       kMinSingleMatchScore, isCanExtendCover) \
  search_and_dispose_cover(covers, diff, sstring, kMinSingleMatchScore, 0,     \
                           isCanExtendCover);
  ...
  static void _fsearch_and_dispose_cover_thread(std::vector<TOldCover> *_covers,
                                                mt_data_t *mt)
  {
    ...
    while (mt->nextBlock(&newRange))
    {
      ...
      first_search_and_dispose_cover(covers, diff_part, *mt->sstring,
                                     mt->kMinSingleMatchScore,
                                     mt->isCanExtendCover);
  ...
  static void first_search_and_dispose_cover_MT(
      std::vector<TOldCover> &covers, const TDiffData &diff,
      const TSuffixString &sstring, int kMinSingleMatchScore,
      ICoverLinesListener *listener, size_t threadNum, bool isCanExtendCover)
  {
    ...
    if ((threadNum > 1) && (diff.oldData != diff.oldData_end) &&
        (newSize >= kMinParallelSize))
    {
      ...
      mt_data.listener =
          (listener && listener->next_search_block_MT) ? listener : 0;
      ...
      if (mt_data.listener && listener->begin_search_block)
        listener->begin_search_block(listener, newSize, mt_data.workBlockSize,
                                     kPartPepeatSize);
      for (size_t i = 0; i < threadCount; i++)
        threads[i] = std::thread(_fsearch_and_dispose_cover_thread,
                                 &threadCovers[i], &mt_data);
      _fsearch_and_dispose_cover_thread(&covers, &mt_data);
      for (size_t i = 0; i < threadCount; i++)
      {
        threads[i].join();
        covers.insert(covers.end(), threadCovers[i].begin(),
                      threadCovers[i].end());
        {
          std::vector<TOldCover> tmp;
          tmp.swap(threadCovers[i]);
        }
      }
  ...
  {
    TDiffResearchCover(TDiffData &diff_, std::vector<TOldCover> &covers_,
                       const TSuffixString &sstring_, int kMinSingleMatchScore_,
                       int kMaxMatchDeep_, bool _isCanExtendCover)
        : diff(diff_), covers(covers_), sstring(sstring_),
          kMinSingleMatchScore(kMinSingleMatchScore_),
          kMaxMatchDeep(kMaxMatchDeep_), limitCoverIndex_back(~(size_t)0),
          limitCoverHitEndPos_back(_kNullCoverHitEndPos),
          isCanExtendCover(_isCanExtendCover)
    {
      researchCover = _researchCover;
    }

    void _researchRange(TDiffLimit *diffLimit)
    {
      search_and_dispose_cover(curCovers, diff, sstring, kMinSingleMatchScore,
                               diffLimit, isCanExtendCover);
      if (curCovers.empty())
        return;
    ...
    inline void doResearchCover(IDiffSearchCoverListener *listener,
                                ...
                                hpatch_StreamPos_t endPosBack,
                                hpatch_StreamPos_t hitPos,
                                hpatch_StreamPos_t hitLen)
    {
      ...
      else
      {
        ...
        if ((!reCovers.empty()) &&
            (reCovers.back().newPos > lastCover_back.newPos))
          lastCover_back = reCovers.back();
      }

      TDiffLimit diffLimit = {listener,
                              cover.newPos + (size_t)hitPos,
                              cover.newPos + (size_t)(hitPos + hitLen),
                              cover.oldPos + (size_t)hitPos,
                              cover.oldPos + (size_t)(hitPos + hitLen),
                              nocover_detect,
                              cover_detect,
                              lastCover_back,
                              kMaxMatchDeep};
    ...
    static void _researchCover(struct IDiffResearchCover *diffi,
                               ...
                               hpatch_StreamPos_t hitLen)
    {
      ...
      self->doResearchCover(listener, limitCoverIndex, endPosBack, hitPos,
                            hitLen);
    }
    ...
    void researchFinish()
    {
      ...
      std::inplace_merge(covers.begin(), covers.begin() + insert, covers.end(),
                         cover_cmp_by_new_t<TOldCover>());
    }
  ...
  {
    inline TDiffInsertCover(std::vector<TOldCover> &_covers) : covers(_covers)
    {
      insertCover = _insertCover;
    }
    static void *_insertCover(IDiffInsertCover *diffi, const void *pInsertCovers,
    ...
    void *_insertCover(const void *pInsertCovers, size_t insertCoverCount,
                       bool insertIsCover32)
    {
      ...
      else
      {
        ...
        for (size_t i = 0; i < insertCoverCount; i++)
        {
          if (insertIsCover32)
          {
            const hpatch_TCover32 &s =
                ((const hpatch_TCover32 *)pInsertCovers)[i];
            covers[oldSize + i] =
                TOldCover((TInt)s.oldPos, (TInt)s.newPos, (TInt)s.length);
          }
          else
          {
                       ...
                       bool isCanExtendCover = true)
  {
      ...
      assert(sizeof(*covers.data()) == sizeof(hpatch_TCover));
    {
      ...
      first_search_and_dispose_cover_MT(covers, diff, *sstring,
                                        kMinSingleMatchScore, listener, threadNum,
                                        isCanExtendCover);
      assert_covers_safe(covers, diff.newData_end - diff.newData,
                         diff.oldData_end - diff.oldData);
      if (listener && listener->search_cover_limit &&
          listener->search_cover_limit(listener, covers.data(), covers.size(),
                                       isCover32))
      {
        TDiffResearchCover diffResearchCover(
            diff, covers, *sstring, kMinSingleMatchScore,
            listener->get_max_match_deep ? listener->get_max_match_deep(listener)
                                         : kDefaultMaxMatchDeepForLimit,
            isCanExtendCover);
        listener->research_cover(listener, &diffResearchCover, covers.data(),
                                 covers.size(), isCover32);
    ...
    if (listener && listener->insert_cover)
    {
      ...
      listener->insert_cover(listener, &diffInsertCover, covers.data(),
                             covers.size(), isCover32, &newDataSize,
                             &oldDataSize);
    ...
    if (listener && listener->search_cover_finish)
    {
      ...
      listener->search_cover_finish(listener, covers.data(), &newCoverCount,
                                    isCover32, &newDataSize, &oldDataSize);
                            ...
                            ICoverLinesListener *listener, size_t threadNum)
{
  ...
  create_compressed_diff(newData, newData_end, oldData, oldData_end, _out_diff,
                         compressPlugin, kMinSingleMatchScore,
                         isUseBigCacheMatch, listener, threadNum);
  checki(out_diff->write(out_diff, 0, _out_diff.data(),
                         _out_diff.data() + _out_diff.size()),
         "create_compressed_diff() out_diff->write");
}

static void serialize_single_compressed_diff(
    const hpatch_TStreamInput *newStream, const hpatch_TStreamInput *oldStream,
    ...
    size_t patchStepMemSize)
{
  ...
  TStepStream stepStream(newStream, oldStream, isZeroSubDiff, covers,
                         patchStepMemSize);
  ...
  TPlaceholder compressed_sizePos =
      outDiff.packUInt_pos(compressPlugin ? stepStream.streamSize : 0);
...
void create_single_compressed_diff(
    ...
    const TByte *oldData_end, std::vector<unsigned char> &out_diff,
    ...
    ICoverLinesListener *listener, size_t threadNum)
{
  ...
  create_single_compressed_diff(newData, newData_end, oldData, oldData_end,
                                &outDiffStream, compressPlugin,
                                kMinSingleMatchScore, patchStepMemSize,
                                isUseBigCacheMatch, listener, threadNum);
}

void create_single_compressed_diff(
    const TByte *newData, const TByte *newData_end, const TByte *oldData,
    const TByte *oldData_end, const hpatch_TStreamOutput *out_diff,
    const hdiff_TCompress *compressPlugin, int kMinSingleMatchScore,
    size_t patchStepMemSize, bool isUseBigCacheMatch,
    ICoverLinesListener *listener, size_t threadNum)
{
                                          ...
                                          const hdiff_TMTSets_s *mtsets)
{
  ...
  serialize_single_compressed_diff(newData, oldData, true, covers, out_diff,
                                   compressPlugin, patchStepMemSize);
}
                           ...
                           hpatch_TDecompress *decompressPlugin)
{
  ...
  mem_as_hStreamInput(&oldStream, oldData, oldData_end);
  mem_as_hStreamInput(&diffStream, diff, diff_end);
  return check_compressed_diff(&newStream, &oldStream, &diffStream,
                               decompressPlugin);
}
                           ...
                           hpatch_TDecompress *decompressPlugin)
{
  ...
  _TCheckOutNewDataStream out_newData(newData, _cache.data(), kACacheBufSize);
  _test_rt(patch_decompress_with_cache(
      &out_newData, oldData, compressed_diff, decompressPlugin,
      _cache.data() + kACacheBufSize, _cache.data_end()));
  _test_rt(out_newData.isWriteFinish());
  return true;
}

bool check_single_compressed_diff(const TByte *newData,
                                  const TByte *newData_end,
                                  const TByte *oldData,
                                  const TByte *oldData_end, const TByte *diff,
                                  const TByte *diff_end,
                                  hpatch_TDecompress *decompressPlugin)
{
  hpatch_TStreamInput newStream;
  hpatch_TStreamInput oldStream;
  ...
  mem_as_hStreamInput(&oldStream, oldData, oldData_end);
  mem_as_hStreamInput(&diffStream, diff, diff_end);
  return check_single_compressed_diff(&newStream, &oldStream, &diffStream,
                                      decompressPlugin);
}
                                  ...
                                  hpatch_TDecompress *decompressPlugin)
{
  hpatch_TStreamInput oldStream;
  mem_as_hStreamInput(&oldStream, oldData, oldData_end);
  return check_single_compressed_diff(newData, &oldStream, diff,
                                      decompressPlugin);
}

static hpatch_BOOL
_check_single_onDiffInfo(struct sspatch_listener_t *listener,
                         ...
                         unsigned char **out_temp_cacheEnd)
{
  ...
  *out_decompressPlugin = (info->compressType[0] == '\0')
                              ? 0
                              : (hpatch_TDecompress *)listener->import;
  return hpatch_TRUE;
}
...
// for test
void __hdiff_private__create_compressed_diff(
    const TByte *newData, const TByte *newData_end, const TByte *oldData,
    const TByte *oldData_end, std::vector<TByte> &out_diff,
    const hdiff_TCompress *compressPlugin, int kMinSingleMatchScore,
    const TSuffixString *sstring)
{
  TDiffData diff;
                               ...
                               const hdiff_TMTSets_s *mtsets)
{
  ...
  TDigestMatcher matcher(oldData, newData, kMatchBlockSize,
                         mtsets ? *mtsets : hdiff_TMTSets_s_kEmpty);
                               ...
                               size_t kMatchBlockSize, size_t threadNum)
{
  ...
  get_match_covers_by_block(&newData_stream, &oldData_stream, out_covers,
                            kMatchBlockSize, &mtsets);
}
...
void get_match_covers_by_sstring(
    ...
    bool isUseBigCacheMatch, ICoverLinesListener *listener, size_t threadNum,
    bool isCanExtendCover)
{
  TDiffData diff;
  std::vector<TOldCover> covers;
  ...
  {
    std::vector<hpatch_TCover_sz> tmp;
    tmp.swap(out_covers);
  }
  get_diff(newData, newData_end, oldData, oldData_end, diff, covers,
           kMinSingleMatchScore, isUseBigCacheMatch, listener, 0, threadNum,
    ...
    bool isCanExtendCover)
{
  ...
  get_match_covers_by_sstring(newData, newData_end, oldData, oldData_end,
                              covers, kMinSingleMatchScore, isUseBigCacheMatch,
                              listener, threadNum, isCanExtendCover);
...
static void stream_serialize_compressed_diff(
    const hpatch_TStreamInput *newData, hpatch_StreamPos_t oldDataSize,
    const hpatch_TStreamOutput *out_diff, const hdiff_TCompress *compressPlugin,
    const TCovers &covers)
{
  ...
  { // empty rle //todo: suport rle data
    if (newData->streamSize > 0)
      packUIntWithTag(rle_ctrlBuf, newData->streamSize - 1, kByteRleType_rle0,
                      kByteRleType_bit);
  ...
  TPlaceholder compress_cover_buf_sizePos = outDiff.packUInt_pos(
      compressPlugin ? cover_buf_size : 0); // compress_cover_buf size
  ...
  TPlaceholder compress_newDataDiff_sizePos = outDiff.packUInt_pos(
      compressPlugin ? newDataDiff_size : 0); // compress_newDataDiff size
  ...
  { // save newDataDiff
    ...
    outDiff.pushStream(&newDataDiff, compressPlugin,
                                   ...
                                   const hdiff_TMTSets_s *mtsets)
{
  TCoversBuf covers(newData->streamSize, oldData->streamSize);
  get_match_covers_by_block(newData, oldData, &covers, kMatchBlockSize, mtsets);
  stream_serialize_compressed_diff(newData, oldData->streamSize, out_diff,
                                   compressPlugin, covers);
}
                            ...
                            hpatch_StreamPos_t out_diff_curPos)
{
  ...
  { // read head
    ...
    if ((decompressPlugin) && (diffInfo.compressedCount > 0))
    {
      checki(decompressPlugin->is_can_open(diffInfo.compressType),
             "resave_compressed_diff() decompressPlugin cannot open compressed "
             "data error!");
    }
  }
  ...
  TPlaceholder compress_cover_buf_sizePos = outDiff.packUInt_pos(
      compressPlugin ? head.cover_buf_size : 0); // compress_cover_buf size
  outDiff.packUInt(head.rle_ctrlBuf_size);       // rle_ctrlBuf size
  TPlaceholder compress_rle_ctrlBuf_sizePos = outDiff.packUInt_pos(
      compressPlugin ? head.rle_ctrlBuf_size : 0); // compress_rle_ctrlBuf size
  outDiff.packUInt(head.rle_codeBuf_size);         // rle_codeBuf size
  TPlaceholder compress_rle_codeBuf_sizePos = outDiff.packUInt_pos(
      compressPlugin ? head.rle_codeBuf_size : 0); // compress_rle_codeBuf size
  outDiff.packUInt(head.newDataDiff_size);
  TPlaceholder compress_newDataDiff_sizePos = outDiff.packUInt_pos(
      compressPlugin ? head.newDataDiff_size : 0); // compress_newDataDiff size

  { // save covers
    TStreamClip clip(in_diff, head.headEndPos, head.coverEndPos,
                     (head.compress_cover_buf_size > 0) ? decompressPlugin : 0,
                     head.cover_buf_size);
  ...
  { // save rle ctrl
    ...
    hpatch_StreamPos_t bufSize =
        isCompressed ? head.compress_rle_ctrlBuf_size : head.rle_ctrlBuf_size;
    TStreamClip clip(in_diff, diffPos0, diffPos0 + bufSize,
                     isCompressed ? decompressPlugin : 0,
  ...
  { // save rle code
    ...
    hpatch_StreamPos_t bufSize =
        isCompressed ? head.compress_rle_codeBuf_size : head.rle_codeBuf_size;
    TStreamClip clip(in_diff, diffPos0, diffPos0 + bufSize,
                     isCompressed ? decompressPlugin : 0,
  ...
  { // save newDataDiff
    ...
    hpatch_StreamPos_t bufSize =
        isCompressed ? head.compress_newDataDiff_size : head.newDataDiff_size;
    TStreamClip clip(in_diff, diffPos0, diffPos0 + bufSize,
                     isCompressed ? decompressPlugin : 0,
...
void resave_single_compressed_diff(
    ...
    hpatch_StreamPos_t in_diff_curPos, hpatch_StreamPos_t out_diff_curPos)
{
  ...
  if (isCompressed)
  { // check
    checki(diffInfo->compressedSize +
                   (in_diff_curPos + diffInfo->diffDataPos) ==
               in_diff->streamSize,
           "resave_single_compressed_diff() diffInfo error!");
    checki((decompressPlugin != 0) &&
               (decompressPlugin->is_can_open(diffInfo->compressType)),
  ...
  { // save single stream data
    TStreamClip clip(in_diff, diffInfo->diffDataPos + in_diff_curPos,
                     in_diff->streamSize, isCompressed ? decompressPlugin : 0,
                     diffInfo->uncompressedSize);
    TPlaceholder compressedSize_pos =
        outDiff.packUInt_pos(compressPlugin ? diffInfo->uncompressedSize : 0);
...
{
  std::vector<unsigned char> out_diff_vec;
  create_diff(newData, newData_end, oldData, oldData_end, out_diff_vec);

  *out_diff_size = out_diff_vec.size();
  *out_diff = (const unsigned char *)malloc(*out_diff_size);
  if (*out_diff != nullptr)
  {
    std::copy(out_diff_vec.begin(), out_diff_vec.end(), *out_diff);
  }
  else
  {
    *out_diff_size = 0;
...
{
  std::vector<unsigned char> out_diff_vec;
  create_compressed_diff(newData, newData_end, oldData, oldData_end, out_diff_vec);
