...
struct TMatchBlock {
  ...
  TMatchBlock(unsigned char *_newData, unsigned char *_newData_end,
      ...
      : newData(_newData), newData_end(_newData_end),
        newData_end_cur(_newData_end), oldData(_oldData),
        oldData_end(_oldData_end), oldData_end_cur(_oldData_end),
        matchBlockSize(_matchBlockSize), threadNum(_threadNum) {}
  void getBlockCovers();
  void getPackedCover();
  void packData();
  void unpackData(IDiffInsertCover *diffi, void *pcovers, size_t coverCount,
                  bool isCover32);
...
protected:
  static void _insert_cover(ICoverLinesListener *listener,
...
template <class _TMatchBlock> struct TCoversOptimMB : public TCoversOptim {
  TCoversOptimMB(unsigned char *newData, unsigned char *newData_end,
      ...
      : TCoversOptim(&_matchBlock),
        _matchBlock(newData, newData_end, oldData, oldData_end, matchBlockSize,
                    threadNum) {
...
void create_compressed_diff_block(
    const hpatch_TStreamInput *newData, // will load in memory
    ...
    size_t matchBlockSize = kDefaultFastMatchBlockSize, size_t threadNum = 1);
void create_compressed_diff_block(
