...
#include "../patch_types.h" //hpatch_packUIntWithTag
...
inline static void packUIntWithTag(std::vector<unsigned char> &out_code,
                                   _UInt uValue, int highTag,
                                   const int kTagBit) {
  ...
  if (!hpatch_packUIntWithTag(&codeEnd, codeBuf + hpatch_kMaxPackedUIntBytes,
                              uValue, highTag, kTagBit))
    throw std::runtime_error(
        "packUIntWithTag<_UInt>() hpatch_packUIntWithTag() error!");
  out_code.insert(out_code.end(), codeBuf, codeEnd);
}

template <class _UInt>
inline static void packUInt(std::vector<unsigned char> &out_code,
                            _UInt uValue) {
...
static void pushBack(std::vector<unsigned char> &out_buf,
...
inline static void pushUInt(std::vector<unsigned char> &out_buf, TUInt v) {
...
inline static void pushUInt(std::vector<unsigned char> &out_buf,
                            unsigned char v) {
  out_buf.insert(out_buf.end(), v);
}

inline static void pushCStr(std::vector<unsigned char> &out_buf,
                            const char *cstr) {
  const unsigned char *data = (const unsigned char *)cstr;
  pushBack(out_buf, data, data + strlen(cstr));
}
inline static void pushString(std::vector<unsigned char> &out_buf,
