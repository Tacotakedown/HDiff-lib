...
hpatch_inline static void _TStreamCacheClip_init(
    TStreamCacheClip *sclip, const hpatch_TStreamInput *srcStream,
    hpatch_StreamPos_t streamPos, hpatch_StreamPos_t streamPos_end,
    unsigned char *aCache, hpatch_size_t cacheSize) {
  assert((streamPos <= streamPos_end) &&
         (streamPos_end <= (srcStream ? srcStream->streamSize : 0)));
...
#define _TStreamCacheClip_isFinish(sclip)                                      \
  (0 == _TStreamCacheClip_leaveSize(sclip))
#define _TStreamCacheClip_isCacheEmpty(sclip)                                  \
  ((sclip)->cacheBegin == (sclip)->cacheEnd)
#define _TStreamCacheClip_cachedSize(sclip)                                    \
  ((hpatch_size_t)((sclip)->cacheEnd - (sclip)->cacheBegin))
...
hpatch_inline static // error return 0
    unsigned char *
    _TStreamCacheClip_accessData(TStreamCacheClip *sclip,
                                 hpatch_size_t readSize) {
...
#define _TStreamCacheClip_skipData_noCheck(sclip, skipSize)                    \
  ((sclip)->cacheBegin += skipSize)
hpatch_BOOL _TStreamCacheClip_skipData(TStreamCacheClip *sclip,
                                       hpatch_StreamPos_t skipLongSize);

hpatch_inline static // error return 0
    unsigned char *
    _TStreamCacheClip_readData(TStreamCacheClip *sclip,
                               hpatch_size_t readSize) {
...
hpatch_BOOL _TStreamCacheClip_addDataTo(TStreamCacheClip *self,
                                        unsigned char *dst,
                                        hpatch_size_t addLen);
...
hpatch_BOOL
_TStreamCacheClip_readType_end(TStreamCacheClip *sclip, unsigned char endTag,
...
static hpatch_inline void
_TOutStreamCache_init(_TOutStreamCache *self,
                      const hpatch_TStreamOutput *dstStream,
...
_TOutStreamCache_resetCache(_TOutStreamCache *self, unsigned char *aCache,
                            hpatch_size_t aCacheSize) {
  assert(0 == self->cacheCur);
  self->cacheBuf = aCache;
  self->cacheEnd = aCacheSize;
}

static hpatch_inline hpatch_StreamPos_t
_TOutStreamCache_leaveSize(const _TOutStreamCache *self) {
  return self->dstStream->streamSize - self->writeToPos;
}
static hpatch_inline hpatch_BOOL
_TOutStreamCache_isFinish(const _TOutStreamCache *self) {
  return self->writeToPos == self->dstStream->streamSize;
}
static hpatch_inline hpatch_size_t
_TOutStreamCache_cachedDataSize(const _TOutStreamCache *self) {
  return self->cacheCur;
}
hpatch_BOOL _TOutStreamCache_flush(_TOutStreamCache *self);
hpatch_BOOL _TOutStreamCache_write(_TOutStreamCache *self,
                                   const unsigned char *data,
                                   hpatch_size_t dataSize);
hpatch_BOOL _TOutStreamCache_fill(_TOutStreamCache *self, hpatch_byte fillValue,
                                  hpatch_StreamPos_t fillLength);

hpatch_BOOL _TOutStreamCache_copyFromClip(_TOutStreamCache *self,
                                          TStreamCacheClip *src,
                                          hpatch_StreamPos_t copyLength);
hpatch_BOOL _TOutStreamCache_copyFromStream(_TOutStreamCache *self,
                                            const hpatch_TStreamInput *src,
                                            hpatch_StreamPos_t srcPos,
                                            hpatch_StreamPos_t copyLength);
hpatch_BOOL _TOutStreamCache_copyFromSelf(_TOutStreamCache *self,
                                          hpatch_StreamPos_t aheadLength,
                                          hpatch_StreamPos_t copyLength);
...
void TDiffToSingleStream_init(TDiffToSingleStream *self,
                              const hpatch_TStreamInput *diffStream);
hpatch_inline static void
TDiffToSingleStream_setInSingleStream(TDiffToSingleStream *self,
                                      hpatch_StreamPos_t singleStreamPos) {
