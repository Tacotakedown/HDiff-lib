  ...
  {                                                                            \
    if (((n) > 0) && (!(stream)->read(stream, pos, dst, dst + (n))))           \
      throw std::runtime_error(                                                \
          "TStreamCache::_resetPos_continue() stream->read() error!");         \
  }

struct TStreamCache {
  TStreamCache(const hpatch_TStreamInput *_stream, unsigned char *_cache,
               size_t _cacheSize, void *_locker)
  ...
  inline const size_t dataLength() const {
    return (size_t)(cacheSize - cachePos);
  }
  inline const unsigned char *data() const { return cache + cachePos; }
  inline bool resetPos(size_t kBackupCacheSize, hpatch_StreamPos_t streamPos,
                       size_t kMinCacheDataSize) {
...
protected:
  inline bool _is_hit_cache(size_t kBackupCacheSize,
                            hpatch_StreamPos_t streamPos,
                            size_t kMinCacheDataSize) {
    return (streamPos + kMinCacheDataSize <= m_readPosEnd) &&
           (streamPos >= m_readPos + kBackupCacheSize);
  }
  inline unsigned char *cachedData() {
    return cache + cacheSize - cachedLength();
  }
  inline const size_t cachedLength() const {
    return (size_t)(m_readPosEnd - m_readPos);
  }
  bool _resetPos_continue(size_t kBackupCacheSize, hpatch_StreamPos_t streamPos,
                          size_t kMinCacheDataSize) {
    // stream:[...     |                |                   |                  |
    // |      ...]
    //             readPos (streamPos-kBackupCacheSize) streamPos
    //             (streamPos+kMinCacheDataSize) (readPosEnd)
    // cache:     [    |                                    |                  |
    // ]
    //           0                                   (init cachePos) cacheSize
    hpatch_StreamPos_t streamSize = stream->streamSize;
    if (streamPos + kMinCacheDataSize > streamSize)
      return false;
    hpatch_StreamPos_t readPos =
        (streamPos >= kBackupCacheSize) ? (streamPos - kBackupCacheSize) : 0;
    size_t readLen = ((streamSize - readPos) >= cacheSize)
                         ? cacheSize
                         : (size_t)(streamSize - readPos);
#if (_IS_USED_MULTITHREAD)
    if (m_locker) {
...
private:
  void _resetPos_continue_read(hpatch_StreamPos_t readPos, size_t readLen) {
    ...
    } else if ((m_readPos < readPos + readLen) &&
               (m_readPosEnd >= readPos + readLen)) {
...
inline static T upperCount(T all_size, Tn node_size) {
  return (all_size + node_size - 1) / node_size;
}

inline static size_t getBackupSize(size_t kMatchBlockSize) {
  return (kMatchBlockSize >= kMinBackupReadSize) ? kMatchBlockSize
                                                 : kMinBackupReadSize;
}
...
TDigestMatcher::TDigestMatcher(const hpatch_TStreamInput *oldData,
    ...
    : m_oldData(oldData), m_newData(newData), m_isUseLargeSorted(true),
      m_mtsets(mtsets), m_newCacheSize(0), m_oldCacheSize(0),
      m_oldMinCacheSize(0), m_backupCacheSize(0), m_kMatchBlockSize(0) {
  ...
  if (oldData->streamSize < kMatchBlockSize)
    return;
  m_kMatchBlockSize = kMatchBlockSize;

  hpatch_StreamPos_t _blockCount =
      getBlockCount(m_kMatchBlockSize, m_oldData->streamSize);
  ...
  if (blockCount != _blockCount)
    throw std::runtime_error(
        "TDataDigest() oldData->streamSize/MatchBlockSize too big error!");
  ...
  m_newCacheSize =
      upperCount(m_kMatchBlockSize * 2 + m_backupCacheSize, kBestReadSize) *
      kBestReadSize;
  m_oldCacheSize =
      upperCount(m_kMatchBlockSize + m_backupCacheSize, kBestReadSize) *
      kBestReadSize;
  m_oldMinCacheSize =
      upperCount(m_kMatchBlockSize + m_backupCacheSize, kMinReadSize) *
      kMinReadSize;
...
struct TIndex_comp {
  ...
  template <class TIndex> inline bool operator()(TIndex x, TIndex y) const {
...
template <bool isMT>
static void _filter_insert(TBloomFilter<adler_hash_t> *filter,
                           const adler_uint_t *begin, const adler_uint_t *end) {
  while (begin != end) {
...
static void filter_insert_parallel(TBloomFilter<adler_hash_t> &filter,
                                   const adler_uint_t *begin,
  ...
  if ((threadNum > 1) && (size >= kInsertMinParallelSize)) {
    ...
    for (size_t i = 0; i < threadCount; i++, begin += step)
      threads[i] =
          std::thread(_filter_insert<true>, &filter, begin, begin + step);
...
#define __sort_indexs(TIndex, indexs, comp, m_threadNum)                       \
  sort_parallel<TIndex, TIndex_comp, 1024 * 8, 137>(                           \
...
void TDigestMatcher::getDigests() {
  ...
  TStreamCache streamCache(m_oldData, m_mem.data(),
                           m_newCacheSize + m_oldCacheSize, 0);
  for (size_t i = 0; i < blockCount; ++i) {
    hpatch_StreamPos_t readPos =
        blockIndexToPos(i, m_kMatchBlockSize, m_oldData->streamSize);
  ...
  filter_insert_parallel(m_filter, m_blocks.data(),
                         m_blocks.data() + blockCount, m_mtsets.threadNum);

  size_t kMaxCmpDeep =
      1 + upperCount(kMinTrustMatchedLength, m_kMatchBlockSize);
  TIndex_comp comp(m_blocks.data(), m_blocks.size(), kMaxCmpDeep);
  if (m_isUseLargeSorted)
    __sort_indexs(std::vector<size_t>::value_type, m_sorted_larger, comp,
                  m_mtsets.threadNum);
  else
...
struct TBlockStreamCache : public TStreamCache {
  TBlockStreamCache(const hpatch_TStreamInput *_stream, unsigned char *_cache,
                    size_t _cacheSize, size_t _backupCacheSize,
                    size_t _kMatchBlockSize, void *_locker)
...
struct TOldStreamCache : public TBlockStreamCache {
  ...
  hpatch_StreamPos_t loop_backward_equal_length(TBlockStreamCache &y) {
    if (!_loop_backward_cache(kMatchBlockSize, 1))
      return 0;
    if (!y._loop_backward_cache(kMatchBlockSize, 1))
      return 0;
    ...
    while (true) {
      ...
      if ((len == xLen) & (cacheSize < maxCacheSize))
        _doGrowCacheSize();
      if (!_loop_backward_cache(len, 1))
        break;
      if (!y._loop_backward_cache(len, 1))
        break;
    }
    return eq_len;
  }
...
private:
  ...
  inline void _doMinCacheSize() {
    ...
    m_readPos =
        (cachedLength() <= cacheSize) ? m_readPos : m_readPosEnd - cacheSize;
  }
  inline void _doGrowCacheSize() {
    const size_t growSize = (cacheSize * 2 <= maxCacheSize)
                                ? cacheSize
...
struct TNewStreamCache : public TBlockStreamCache {
  TNewStreamCache(const hpatch_TStreamInput *_stream, unsigned char *_cache,
                  size_t _cacheSize, size_t _backupCacheSize,
                  size_t _kMatchBlockSize, void *_locker)
      : TBlockStreamCache(_stream, _cache, _cacheSize, _backupCacheSize,
                          _kMatchBlockSize, _locker) {
    resetPos(0);
  }
  void toBestDataLength() {
    if (dataLength() * 2 >= cacheSize)
      return;
  ...
  inline bool resetPos(hpatch_StreamPos_t streamPos) {
    if (!TBlockStreamCache::resetPos(streamPos))
      return false;
  ...
  inline bool roll() {
    ...
    if (dataLength() > kMatchBlockSize) {
      ...
      roll_digest = adler_roll(roll_digest, kMatchBlockSize, cur_datas[0],
                               cur_datas[kMatchBlockSize]);
      ++cachePos;
      return true;
    } else {
      if (!TBlockStreamCache::resetPos(pos() + 1))
        return false;
      --cachePos;
      return roll();
    }
...
struct TDigest_comp_i : public TDigest_comp {
  inline TDigest_comp_i(const adler_uint_t *_blocks, size_t _n)
      : TDigest_comp(_blocks), n(_n), i(0) {}
  template <class TIndex>
  inline bool operator()(const TIndex &x, const TDigest &y) const {
...
static hpatch_StreamPos_t getMatchLength(TOldStreamCache &oldStream,
                                         TNewStreamCache &newStream,
                                         hpatch_StreamPos_t *pOldPos,
                                         size_t kMatchBlockSize,
  ...
  if (oldStream.resetPos(*pOldPos) &&
    ...
    hpatch_StreamPos_t beq_len =
        oldStream.loop_backward_equal_length(newStream);
...
template <class TIndex>
static const TIndex *
getBestMatchi(const adler_uint_t *blocksBase, size_t blocksSize,
  ...
  if (max_digests_n > _data_max_digests_n)
    max_digests_n = _data_max_digests_n;
  ...
  digests_eq_n = 1;
  //缩小[left best
  // right)范围,留下最多2个(因为签名匹配并不保证一定相等,2个的话应该就够了?);
  if (right - left > 1) {
    ...
    for (; (digests_eq_n < max_digests_n) && (right - left > 1);
         ++digests_eq_n, bdata += kMatchBlockSize) {
  ...
  hpatch_StreamPos_t linkOldPos =
      newStream.pos() + lastCover.oldPos - lastCover.newPos;
  TIndex linkIndex =
      (TIndex)posToBlockIndex(linkOldPos, kMatchBlockSize, blocksSize);
  ...
  if (best == 0) {
    ...
    if (findCount > kMaxLinkIndexFindCount)
    ...
    for (TIndex inc = 1; (inc <= findCount); ++inc) { // linkIndex附近找;
      ...
      if (inc & 1) {
        ...
        if (linkIndex + s >= blocksSize)
          continue;
        fi = linkIndex + s;
      }
      std::pair<const TIndex *, const TIndex *> i_range =
          std::equal_range(left, right, fi, comp);
  ...
  if (best == 0) { //继续找;
    ...
    const TIndex *end =
        (left + kBestMatchRange >= right) ? right : (left + kBestMatchRange);
    for (const TIndex *it = left; it < end; ++it) {
      hpatch_StreamPos_t oldIndex = (*it);
      hpatch_StreamPos_t distance = (oldIndex < linkIndex)
                                        ? (linkIndex - oldIndex)
                                        : (oldIndex - linkIndex);
...
template <class TIndex>
static bool getBestMatch(const TIndex *left, const TIndex *right,
                         const TIndex *best, TOldStreamCache &oldStream,
                         TNewStreamCache &newStream, const TCover &lastCover,
                         size_t digests_eq_n, TCover *out_curCover) {
  assert(best != 0);
  const size_t kMatchBlockSize = newStream.kMatchBlockSize;
  const hpatch_StreamPos_t newPos = newStream.pos();
  ...
  size_t findCount = (right - left) * 2 + 1;
  if (findCount > kMaxFindCount)
    findCount = kMaxFindCount;
  for (size_t inc = 1; (inc <= findCount); ++inc) { // best附近找;
    ...
    hpatch_StreamPos_t oldPos =
        blockIndexToPos(*fi, kMatchBlockSize, oldStream.streamSize());
    hpatch_StreamPos_t matchedOldPos = oldPos;
    hpatch_StreamPos_t curEqLen = getMatchLength(
        oldStream, newStream, &matchedOldPos, kMatchBlockSize, lastCover);
    if (curEqLen > bestLen) {
      isMatched = true;
      bestLen = curEqLen;
...
static const size_t getOldPosCost(hpatch_StreamPos_t oldPos,
                                  const TCover &lastCover) {
  hpatch_StreamPos_t oldPosEnd = lastCover.oldPos + lastCover.length;
  hpatch_StreamPos_t dis =
      (oldPos >= oldPosEnd) ? (oldPos - oldPosEnd) : (oldPosEnd - oldPos);
...
static void tryLink(const TCover &lastCover, TCover &matchCover,
  ...
  if (lastCover.length <= 0)
    return;
  hpatch_StreamPos_t linkOldPos =
      matchCover.newPos + lastCover.oldPos - lastCover.newPos;
  if (linkOldPos == matchCover.oldPos)
    return;
  newStream.TBlockStreamCache::resetPos(matchCover.newPos);
  hpatch_StreamPos_t matchedOldPos = linkOldPos;
  hpatch_StreamPos_t curEqLen =
      getMatchLength(oldStream, newStream, &matchedOldPos,
...
template <class TIndex>
static void tm_search_cover(
    const adler_uint_t *blocksBase, const TIndex *iblocks,
    const TIndex *iblocks_end, TOldStreamCache &oldStream,
    TNewStreamCache &newStream, const TBloomFilter<adler_hash_t> &filter,
    hpatch_TOutputCovers *out_covers, hpatch_StreamPos_t _coverNewOffset,
  ...
  while (true) {
    ...
    typename TDigest_comp::TDigest digest_value(digest);
    std::pair<const TIndex *, const TIndex *> range =
        std::equal_range(iblocks, iblocks_end, digest_value, comp);
    ...
    size_t digests_eq_n;
    const TIndex *besti =
        getBestMatchi(blocksBase, blocksSize, &range.first, &range.second,
                      newStream, lastCover, &digests_eq_n);
    {
#if (_IS_USED_MULTITHREAD)
      CAutoLocker _autoDataLocker(
          _mtsets.oldDataIsMTSafe
              ? 0
      ...
      if (getBestMatch(range.first, range.second, besti, oldStream, newStream,
        ...
        if (curCover.length >= kMinMatchedLength) { // matched
          TCover _cover;
          setCover(_cover, curCover.oldPos, curCover.newPos + _coverNewOffset,
                   curCover.length);
          {
#if (_IS_USED_MULTITHREAD)
            CAutoLocker _autoCoverLocker(_mtsets.oldDataIsMTSafe ? _dataLocker
                                                                 : 0);
#endif
            if (!out_covers->push_cover(out_covers, &_cover))
              throw std::runtime_error(
                  "TDigestMatcher::search_cover() push_cover error!");
          }
          lastCover = curCover;
    ...
    // next
    if (!newStream.resetPos(newPosNext))
      break; // finish
  }
}

#define __search_cover(indexs, coverNewOffset, newLocker, dataLocker)          \
  tm_search_cover(m_blocks.data(), indexs.data(),                              \
                  indexs.data() + indexs.size(), oldStream, newStream,         \
                  m_filter, out_covers, coverNewOffset, newLocker, dataLocker, \
                  m_mtsets)

void TDigestMatcher::_search_cover(const hpatch_TStreamInput *newData,
                                   hpatch_StreamPos_t newOffset,
                                   hpatch_TOutputCovers *out_covers,
                                   unsigned char *pmem, void *newDataLocker,
                                   void *dataLocker) {
  TNewStreamCache newStream(newData, pmem, m_newCacheSize, m_backupCacheSize,
                            m_kMatchBlockSize,
                            m_mtsets.newDataIsMTSafe ? 0 : newDataLocker);
  TOldStreamCache oldStream(m_oldData, pmem + m_newCacheSize, m_oldMinCacheSize,
                            m_oldCacheSize, m_backupCacheSize,
                            m_kMatchBlockSize, 0);
  if (m_isUseLargeSorted)
...
void TDigestMatcher::_search_cover_thread(hpatch_TOutputCovers *out_covers,
  ...
  const hpatch_StreamPos_t rollCount =
      m_newData->streamSize - (m_kMatchBlockSize - 1);
  std::atomic<uint_work> &workIndex = *(std::atomic<uint_work> *)&mt.workIndex;
  while (true) {
    uint_work curWorkIndex = workIndex++;
    if (curWorkIndex >= workCount)
      break;
    hpatch_StreamPos_t new_begin = rollCount * curWorkIndex / workCount;
    hpatch_StreamPos_t new_end =
        (curWorkIndex + 1 < workCount)
            ? rollCount * (curWorkIndex + 1) / workCount
            : rollCount;
    assert(new_end + kPartPepeatSize <= m_newData->streamSize);
    TStreamInputClip newClip;
    TStreamInputClip_init(&newClip, m_newData, new_begin,
                          new_end + kPartPepeatSize);
...
static inline void __search_cover_mt(TDigestMatcher *self,
                                     hpatch_TOutputCovers *out_covers,
                                     unsigned char *pmem, void *mt_data) {
...
void TDigestMatcher::search_cover(hpatch_TOutputCovers *out_covers) {
  if (m_blocks.empty())
    return;
  if (m_newData->streamSize < m_kMatchBlockSize)
    return;
#if (_IS_USED_MULTITHREAD)
  ...
  if (threadNum > 1) {
    const hpatch_StreamPos_t rollCount =
        m_newData->streamSize - (m_kMatchBlockSize - 1);
    size_t bestStep = (kBestParallelSize / 2 > m_kMatchBlockSize)
                          ? kBestParallelSize
                          : 2 * m_kMatchBlockSize;
    ...
    if ((sizeof(mt_data.workCount) < sizeof(workCount)) &&
        (mt_data.workCount != workCount))
      throw std::runtime_error(
          "TDigestMatcher::search_cover() muti-thread workCount error!");
    mt_data.workIndex = 0;
    const size_t threadCount = threadNum - 1;
    std::vector<std::thread> threads(threadCount);
    unsigned char *pmem = m_mem.data();
    for (size_t i = 0; i < threadCount;
         i++, pmem += (m_newCacheSize + m_oldCacheSize))
      threads[i] =
          std::thread(__search_cover_mt, this, out_covers, pmem, &mt_data);
