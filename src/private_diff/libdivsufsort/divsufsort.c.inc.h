...
static void _sssort_thread(saint_t *c0, saint_t *c1, saidx_t *j, sastore_t *buf,
                           mt_data_t *mt) {
  saidx_t k = 0;
  saidx_t l;
  ...
  for (;;) {
    ...
    sastore_t *SA = mt->SA;
    sssort(mt->T, mt->PAb, SA + k, SA + l, buf, mt->bufsize, 2, mt->n,
           *(SA + k) == (mt->m - 1));
  }
}
#endif
...
static saidx_t sort_typeBstar(const sauchar_t *T, sastore_t *SA,
...
static void construct_SA(const sauchar_t *T, sastore_t *SA, saidx_t *bucket_A,
...
saint_t divsufsort(const sauchar_t *T, sastore_t *SA, saidx_t n,
                   int threadNum) {
...
const char *divsufsort_version(void) { return PROJECT_VERSION_FULL; }