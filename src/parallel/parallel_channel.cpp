...
public:
  explicit _CChannel_import(ptrdiff_t maxDataCount)
      : _locker(0), _sendCond(0), _acceptCond(0), _maxDataCount(maxDataCount),
        _waitingCount(0), _isClosed(false) {
    _locker = locker_new();
    ...
    assert(data != 0);
    {
      ...
      while (true) {
        if (_isClosed)
          return false;
        if ((_maxDataCount <= 0) ||
            (_dataList.size() < (size_t)_maxDataCount)) {
          try {
...
CChannel::~CChannel() { delete (_CChannel_import *)_import; }
void CChannel::close() { _import->close(); }
...
TChanData CChannel::accept(bool isWait) { return _import->accept(isWait); }
