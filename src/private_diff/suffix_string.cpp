...
template <class T> static void _clearVector(std::vector<T> &v) {
  std::vector<T> _tmp;
  v.swap(_tmp);
}
...
static bool getStringIsLess(const TChar *str0, const TChar *str0End,
  ...
  const int kMaxCmpLength_sort =
      1024 *
      4; //警告:这是一个特殊的处理手段,用以避免_suffixString_create在使用std::sort时
         //  某些情况退化到O(n*n)复杂度(运行时间无法接受),设置最大比较长度从而控制算法在
         //  O(kMaxCmpLength_sort*n)复杂度以内,这时排序的结果并不是标准的后缀数组;
  if (L0 > kMaxCmpLength_sort)
    L0 = kMaxCmpLength_sort;
  if (L1 > kMaxCmpLength_sort)
    L1 = kMaxCmpLength_sort;
#endif
  TInt LMin;
  if (L0 < L1)
    LMin = L0;
...
struct StringToken {
  const TChar *begin;
  const TChar *end;
  inline explicit StringToken(const TChar *_begin, const TChar *_end)
      : begin(_begin), end(_end) {}
};

class TSuffixString_compare {
public:
  inline TSuffixString_compare(const TChar *begin, const TChar *end)
...
static void _suffixString_create(const TChar *src, const TChar *src_end,
  ...
  int rt = 0;
  try {
    std::sort<TSAInt *, const TSuffixString_compare &>(
        &out_sstring[0], &out_sstring[0] + size,
  ...
  if (sizeof(TSAInt) == 8)
    ...
    rt = divsufsort(src, (saidx32_t *)&out_sstring[0], (saidx32_t)size,
                    (int)threadNum);
#endif
  if (rt != 0)
    throw std::runtime_error("suffixString_create() error.");
...
#else
template <class T> inline static T *__select_mid(T *&p, size_t n) {
  return p + (n >> 1);
}
...
inline static const T *_lower_bound(const T *rbegin, const T *rend,
                                    ...
                                    const TChar *src_end, size_t min_eq = 0) {
#ifdef _SA_MATCHBY_STD_LOWER_BOUND
  return std::lower_bound<const T *, StringToken,
                          const TSuffixString_compare &>(
      rbegin, rend, StringToken(str, str_end),
      TSuffixString_compare(src_begin, src_end));
#else
  ...
  while (size_t len = (size_t)(rend - rbegin)) {
    ...
    while (true) {
      ...
      if (!sub) {
        ...
        if (eq_len < kMaxCmpLength_forLimitRangeDiff) // only for optimize
                                                      // limitRange match speed
          continue;
        else
          return mid;
      } else {
...
static TInt _lower_bound_TInt(const TInt *rbegin, const TInt *rend,
  ...
  return _lower_bound(rbegin, rend, str, str_end, src_begin, src_end, min_eq) -
         SA_begin;
}
...
TSuffixString::TSuffixString(bool isUsedFastMatch)
    : m_src_begin(0), m_src_end(0), m_isUsedFastMatch(isUsedFastMatch),
      m_cached2char_range(0) {
  clear_cache();
}

TSuffixString::TSuffixString(const TChar *src_begin, const TChar *src_end,
                             bool isUsedFastMatch, size_t threadNum)
    : m_src_begin(0), m_src_end(0), m_isUsedFastMatch(isUsedFastMatch),
      m_cached2char_range(0) {
  clear_cache();
...
TSuffixString::~TSuffixString() { clear(); }
...
void TSuffixString::resetSuffixString(const TChar *src_begin,
                                      const TChar *src_end, size_t threadNum) {
...
TInt TSuffixString::lower_bound(const TChar *str, const TChar *str_end) const {
  ...
  if (m_isUsedFastMatch &&
      (!m_fastMatch.isHit(TFastMatchForSString::getHash(str))))
    return -1;
...
#define kMinStrLen 2
  ...
  if ((kMinStrLen >= 2) & (m_cached2char_range != 0)) {
    ...
    return m_lower_bound((TChar *)m_cached_SA_begin + r0,
                         (TChar *)m_cached_SA_begin + r1, str, str_end,
                         m_src_begin, m_src_end, m_cached_SA_begin, 2);
  } else if (kMinStrLen > 0) {
    size_t c = str[0];
    return m_lower_bound(m_cached1char_range[c], m_cached1char_range[c + 1],
                         str, str_end, m_src_begin, m_src_end,
                         m_cached_SA_begin, 1);
  } else {
    return -1;
  }
...
void TSuffixString::build_cache(size_t threadNum) {
...
#if (_SSTRING_FAST_MATCH > 0)
  if (m_isUsedFastMatch)
    m_fastMatch.buildMatchCache(m_src_begin, m_src_end, threadNum);
#endif
  const size_t kUsedCacheMinSASize =
      2 * (1 << 20); //当字符串较大时再启用大缓存表.
  if (SASize() > kUsedCacheMinSASize) {
    m_cached2char_range =
        new TChar[(256 * 256 + 1) *
                  (isUseLargeSA() ? sizeof(size_t) : sizeof(TInt32))];
  }
  ...
  if (isUseLargeSA()) {
    ...
    _build_range256((TInt *)m_cached_SA_begin, (TInt *)m_cached_SA_end,
                    m_src_begin, m_src_end,
                    (const TInt **)&m_cached1char_range[0]);
    if (m_cached2char_range) {
    ...
    _build_range256((TInt32 *)m_cached_SA_begin, (TInt32 *)m_cached_SA_end,
                    m_src_begin, m_src_end,
                    (const TInt32 **)&m_cached1char_range[0]);
    if (m_cached2char_range) {
...
void TFastMatchForSString::buildMatchCache(const TChar *src_begin,
                                           const TChar *src_end,
                                           size_t threadNum) {
  ...
  if (srcSize >= kFMMinStrSize) {
    ...
    if ((threadNum > 1) && (rollSize >= kInsertMinParallelSize)) {
      ...
      for (size_t i = 0; i < threadCount; i++, src_begin += step)
        threads[i] = std::thread(_filter_insert<true>, &bf, src_begin,
                                 src_begin + step + (kFMMinStrSize - 1));
