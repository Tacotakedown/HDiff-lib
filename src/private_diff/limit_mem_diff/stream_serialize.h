...
private:
  ...
  static hpatch_BOOL _read(const hpatch_TStreamInput *stream,
...
private:
  ...
  static hpatch_BOOL _read(const struct hpatch_TStreamInput *stream,
                           hpatch_StreamPos_t readFromPos,
                           unsigned char *out_data,
                           unsigned char *out_data_end);
};
...
struct TNewDataDiffStream : public hpatch_TStreamInput {
  inline TNewDataDiffStream(const TCovers &_covers,
                            const hpatch_TStreamInput *_newData,
                            hpatch_StreamPos_t newDataDiff_size)
      : covers(_covers), newData(_newData) {
    _init(newDataDiff_size);
  }
  inline TNewDataDiffStream(const TCovers &_covers,
                            const hpatch_TStreamInput *_newData)
      : covers(_covers), newData(_newData) {
    _init(getDataSize(_covers, _newData->streamSize));
  }
  inline TNewDataDiffStream(const TCovers &_covers,
                            const hpatch_TStreamInput *_newData, size_t coveri,
                            hpatch_StreamPos_t newDataPos,
                            hpatch_StreamPos_t newDataPosEnd)
      : covers(_covers), newData(_newData) {
    _initByRange(coveri, newDataPos, newDataPosEnd);
  }
  static hpatch_StreamPos_t getDataSize(const TCovers &covers,
...
private:
  ...
  getDataSizeByRange(const TCovers &covers, size_t coveri,
                     hpatch_StreamPos_t newDataPos,
                     hpatch_StreamPos_t newDataPosEnd);
  void _init(hpatch_StreamPos_t newDataDiff_size);
  void _initByRange(size_t coveri, hpatch_StreamPos_t newDataPos,
                    hpatch_StreamPos_t newDataPosEnd);
  const TCovers &covers;
...
private:
  ...
  hpatch_BOOL readTo(hpatch_StreamPos_t readFromPos, unsigned char *out_data,
                     unsigned char *out_data_end);
  static hpatch_BOOL _read(const hpatch_TStreamInput *stream,
                           hpatch_StreamPos_t readFromPos,
                           unsigned char *out_data,
...
public:
  ...
  inline TStreamClip(const hpatch_TStreamInput *stream,
                     ...
                     hpatch_StreamPos_t uncompressSize = 0)
      : _decompressHandle(0) {
    reset(stream, clipBeginPos, clipEndPos, decompressPlugin, uncompressSize);
  }
  void reset(const hpatch_TStreamInput *stream, hpatch_StreamPos_t clipBeginPos,
             hpatch_StreamPos_t clipEndPos,
             hpatch_TDecompress *decompressPlugin = 0,
             hpatch_StreamPos_t uncompressSize = 0);
...
private:
  ...
  static hpatch_BOOL _clip_read(const hpatch_TStreamInput *stream,
                                hpatch_StreamPos_t readFromPos,
                                unsigned char *out_data,
                                unsigned char *out_data_end);
};
...
struct TStepStream : public hpatch_TStreamInput {
  TStepStream(const hpatch_TStreamInput *newStream,
              const hpatch_TStreamInput *oldStream, bool isZeroSubDiff,
...
struct TDiffStream {
  ...
  hpatch_StreamPos_t pushStream(
      const hpatch_TStreamInput *stream, const hdiff_TCompress *compressPlugin,
      const TPlaceholder &update_compress_sizePos, bool isMustCompress = false,
      const hpatch_StreamPos_t cancelSizeOnCancelCompress = 0);
  hpatch_StreamPos_t
  pushStream(const hpatch_TStreamInput *stream,
             const hdiff_TCompress *compressPlugin, bool isMustCompress = false,
             const hpatch_StreamPos_t cancelSizeOnCancelCompress = 0) {
    TPlaceholder nullPos(0, 0);
    return pushStream(stream, compressPlugin, nullPos, isMustCompress,
                      cancelSizeOnCancelCompress);
  }
...
struct TVectorAsStreamOutput : public hpatch_TStreamOutput {
  ...
  static hpatch_BOOL _write(const hpatch_TStreamOutput *stream,
    ...
    if (writeToPos > dst.size())
      return hpatch_FALSE;
    if (dst.size() == writeToPos) {
      dst.insert(dst.end(), data, data_end);
    } else {
      if (writeToPos + writeLen != (size_t)(writeToPos + writeLen))
        return hpatch_FALSE;
  ...
  static hpatch_BOOL _read(const struct hpatch_TStreamOutput *stream,
                           hpatch_StreamPos_t readFromPos,
                           unsigned char *out_data,
                           unsigned char *out_data_end) {
    const TVectorAsStreamOutput *self =
        (const TVectorAsStreamOutput *)stream->streamImport;
    const std::vector<unsigned char> &src = self->dst;
    size_t readLen = out_data_end - out_data;
    if (readFromPos + readLen > src.size())
      return hpatch_FALSE;
...
#define _test_rt(value)                                                        \
  {                                                                            \
    if (!(value)) {                                                            \
      LOG_ERR("check " #value " error!\n");                                    \
      return hpatch_FALSE;                                                     \
    }                                                                          \
  }

struct _TCheckOutNewDataStream : public hpatch_TStreamOutput {
  _TCheckOutNewDataStream(const hpatch_TStreamInput *_newData,
...
private:
  ...
  unsigned char *buf;
  size_t bufSize;
  static hpatch_BOOL _read_writed(const struct hpatch_TStreamOutput *stream,
                                  hpatch_StreamPos_t readFromPos,
                                  unsigned char *out_data,
                                  unsigned char *out_data_end);
  static hpatch_BOOL _write_check(const hpatch_TStreamOutput *stream,
...
static inline void do_compress(std::vector<unsigned char> &out_code,
